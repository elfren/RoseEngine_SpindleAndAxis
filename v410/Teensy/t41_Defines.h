...
struct configPageRec // page 5,6,13,14
{
	...
	float grow_AxisChange;
	float grow_SpindleChange;
};
...
struct configPageOne  //pageOne
{
	...
	int32_t accel_M3;
	int32_t accel_M4;
	int32_t axisId; // Z:1, X:2, M3:3, M4:4,Spindle:0
};
...
struct configPageGreekKey
{
	...
	// pageGrk settings
	uint32_t patternId;
	float countPatternPer360_Grk;
	uint32_t patternCount_Grk;
	uint32_t radialOrAxial_Pattern;
	float segmentLength_Grk;

	// pageProgram settings
	uint32_t patternCount_Program;
	float countPatternPer360_Program;
	uint32_t segmentOrActual;
	uint32_t radialOrAxial_File;
	float segmentLength_Program;
...
unsigned int eePromAddress_One = 1750;  // /eeProm address for pageOne
...
uint32_t storedSpeedPercent = 1;
uint32_t currentSpeed_Axis = 0;
uint32_t currentSpeed_Spindle = 0;
