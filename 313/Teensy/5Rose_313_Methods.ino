	...
	switch (axisId)
	{
		case ID_SPINDLE:
		{
			...
			{			
				if(!ignoreDisable)
				{ 
					if (configSetup.keepSpindleEnabled==0)
					{ 
		...
		case ID_AXIS_Z:
		{
			if (enabled)
			{
				digitalWrite(PIN_AXIS_Z_ENABLE, configSetup.polarity_Z ? (LOW) : (HIGH)); // Enable 
				SerialPrint("pageSplash.vaAxisEnabledZ.val=1"); // 1 = enabled
				SerialPrint("\xFF\xFF\xFF");

				//SerialPrint("pageMore.c5.val=1");
				//SerialPrint(nextionEnd);
			}
			else
			{
				if (!ignoreDisable)
				{
					if (configSetup.keepZEnabled == 0)
					{
						digitalWrite(PIN_AXIS_Z_ENABLE, configSetup.polarity_Z ? (HIGH) : (LOW)); // Disable 
						SerialPrint("pageSplash.vaAxisEnabledZ.val=0"); // 1 = enabled
						SerialPrint("\xFF\xFF\xFF");

						//SerialPrint("pageMore.c5.val=0");
						//SerialPrint(nextionEnd);
					}
					else
					{
		...
		case ID_AXIS_X:
		{
			if (enabled)
			{
				digitalWrite(PIN_AXIS_X_ENABLE, configSetup.polarity_X ? (LOW) : (HIGH)); // Enable 
			...
			else
			{
				if (!ignoreDisable)
				{
					if (configSetup.keepXEnabled == 0)
					{
						digitalWrite(PIN_AXIS_X_ENABLE, configSetup.polarity_X ? (HIGH) : (LOW)); // Disable 
		...
		case ID_AXIS_3:
		{
			if (enabled)
			{
				digitalWrite(PIN_AXIS_3_ENABLE, configSetup.polarity_M3 ? (LOW) : (HIGH)); // Enable 
			...
			else
			{
				if (!ignoreDisable)
				{
					if (configSetup.keepM3Enabled == 0)
					{
						digitalWrite(PIN_AXIS_3_ENABLE, configSetup.polarity_M3 ? (HIGH) : (LOW)); // Disable 
		...
		case ID_AXIS_4:
		{
			...
			Serial.print("M4.currentPinState: ");
			Serial.println(currentPinState);

			if (enabled)
			{
				digitalWrite(PIN_AXIS_4_ENABLE, configSetup.polarity_M4 ? (LOW) : (HIGH)); // Enable 
				digitalWriteFast(PIN_AXIS_4_ENABLE, configSetup.polarity_M4 ? (LOW) : (HIGH)); // Enable 
				SerialPrint("pageSplash.vAxisEnabledM4.val=1"); // 1 = enabled
				SerialPrint("\xFF\xFF\xFF");
				Serial.println("1.vAxisEnabledM4.val=1");
			...
			else
			{
				if (!ignoreDisable)
				{
					if (configSetup.keepM4Enabled == 0)
					{
						digitalWrite(PIN_AXIS_4_ENABLE, configSetup.polarity_M4 ? (HIGH) : (LOW)); // Disable 
						SerialPrint("pageSplash.vAxisEnabledM4.val=0");
						SerialPrint("\xFF\xFF\xFF");
						Serial.println("2.vAxisEnabledM4.val=0");
					}
					else
					{
						digitalWrite(PIN_AXIS_4_ENABLE, configSetup.polarity_M4 ? (LOW) : (HIGH)); // Enable 
						SerialPrint("pageSplash.vAxisEnabledM4.val=1"); // 1 = enabled
						SerialPrint("\xFF\xFF\xFF");
						Serial.println("3.vAxisEnabledM4.val=1");
					}
				}
			}
...
float StepsToDegrees(float steps, int axisId)
{
	...
	if (axisId == ID_AXIS_3)
	{
		retVal = (steps / (configSetup.steps360_M3 * configSetup.microsteps_M3 * configSetup.gearRatio_M3)) * 360;
	}
...
float RevolutionsToSteps(float revolutions, int direction, int axisId)
{
	...
	if (axisId == ID_AXIS_3)
	{
		retVal = revolutions * configSetup.gearRatio_M3 * configSetup.microsteps_M3 * configSetup.steps360_M3 * direction;
	}
	else if (axisId == ID_AXIS_4)
	{
		retVal = revolutions * configSetup.gearRatio_M4 * configSetup.microsteps_M4 * configSetup.steps360_M4 * direction;
	}
...
float StepsToDistance_Axis(float steps, int axisId)
{
	...
	switch (axisId)
	{
		case ID_AXIS_Z:
		{
			retVal = (steps * configSetup.distancePerRev_Z) / (configSetup.steps360_Z * configSetup.microsteps_Z);
			break;
		}
		case ID_AXIS_X:
		{
			...
			{ 
				retVal = (steps * configSetup.distancePerRev_X) / (configSetup.steps360_X * configSetup.microsteps_X);
			}
			else
			{
				retVal = (steps * configSetup.distancePerRev_XAlt) / (configSetup.steps360_XAlt * configSetup.microsteps_XAlt);
			}
			break;
		}
		case ID_AXIS_3: // Rotary axis
		{
			switch (configSetup.radialOrLinear_M3)
			{
				case RADIAL_M3:
				{
					retVal = (steps / (configSetup.steps360_M3 * configSetup.microsteps_M3 * configSetup.gearRatio_M3)) * 360;
				...
				case LINEAR_M3:
				{
					retVal = (steps * configSetup.distancePerRev_M3) / (configSetup.steps360_M3 * configSetup.microsteps_M3);
				}
			}
			break;
		}
		...
		{
			switch (configSetup.radialOrLinear_M4)
			{
			case RADIAL_M4:
			{
				retVal = (steps / (configSetup.steps360_M3 * configSetup.microsteps_M3 * configSetup.gearRatio_M3)) * 360;
			...
			case LINEAR_M4:
			{
				retVal = (steps * configSetup.distancePerRev_M4) / (configSetup.steps360_M4 * configSetup.microsteps_M4);
			}
			}
			break;
		}
...
float DistanceToSteps_M3Radial(float linearDistance)
{
	...
	float stepsPer360 = configSetup.steps360_M3 * configSetup.microsteps_M3 * configSetup.gearRatio_M3;
	...
	Serial.println(configSetup.gearRatio_M3);
...
float DistanceToSteps_M4Radial(float linearDistance)
{
	...
	float stepsPer360 = configSetup.steps360_M4* configSetup.microsteps_M4* configSetup.gearRatio_M4;
	...
	Serial.println(configSetup.gearRatio_M4);
	Serial.print("circumference: ");
	Serial.println(circumference);
	Serial.print("linearDistance: ");
	Serial.println(linearDistance);
...
{
	float retVal = (distance / (configSetup.distancePerRev_M3) * (configSetup.steps360_M3 * configSetup.microsteps_M3));
#ifdef VERBOSE
...
{
	float retVal = (distance / (configSetup.distancePerRev_M4) * (configSetup.steps360_M4 * configSetup.microsteps_M4));
#ifdef VERBOSE
	Serial.print("distance: ");
	Serial.println(distance);
...
float DistanceToSteps_Axis(float distance, int axisId)
{
	...
	switch (axisId)
	{
		case ID_AXIS_Z:
		{
			retVal = (distance / configSetup.distancePerRev_Z)* (configSetup.steps360_Z * configSetup.microsteps_Z);
			break;
		}
		case ID_AXIS_X:
		{
			if (configSetup.xAltX == 0)
			{
				retVal = (distance / configSetup.distancePerRev_X) * (configSetup.steps360_X * configSetup.microsteps_X);
			}
			else
			{
				retVal = (distance / configSetup.distancePerRev_XAlt) * (configSetup.steps360_XAlt * configSetup.microsteps_XAlt);
			}
			break;
		}
		case ID_AXIS_3:
...
//	const char* position_Char = "Position:";
	...
	switch (axisId)
	{
		case ID_MOVE_AXIS_Z1:
		{
			...
			stepsToMove = (configMove.distance_MoveZ1 / (configSetup.distancePerRev_Z) * (configSetup.steps360_Z * configSetup.microsteps_Z));
		...
		case ID_MOVE_AXIS_Z2:
		{
			...
			stepsToMove = (configMove.distance_MoveZ2 / (configSetup.distancePerRev_Z) * (configSetup.steps360_Z * configSetup.microsteps_Z));
		...
		case ID_MOVE_AXIS_X1:
		{
			...
			if (configSetup.xAltX == 0)
			{
				stepsToMove = (configMove.distance_MoveX1 / (configSetup.distancePerRev_X) * (configSetup.steps360_X * configSetup.microsteps_X));
			}
			else
			{
				stepsToMove = (configMove.distance_MoveX1 / (configSetup.distancePerRev_XAlt) * (configSetup.steps360_XAlt * configSetup.microsteps_XAlt));
			}
		...
		case ID_MOVE_AXIS_X2:
		{
			...
			if (configSetup.xAltX == 0)
			{
				stepsToMove = (configMove.distance_MoveX2 / (configSetup.distancePerRev_X) * (configSetup.steps360_X * configSetup.microsteps_X));
			}
			else
			{
				stepsToMove = (configMove.distance_MoveX2 / (configSetup.distancePerRev_XAlt) * (configSetup.steps360_XAlt * configSetup.microsteps_XAlt));
			}
		...
		case ID_MOVE_AXIS_M3_1:
		{
			...
			if (configSetup.radialOrLinear_M3 == RADIAL_M3)
			{
		...
		case ID_MOVE_AXIS_M4_1:
		{
			...
			if (configSetup.radialOrLinear_M4 == RADIAL_M4)
			{
...
void OneRunStepper(int direction) // pageMulti
{
	...
	switch (configMulti.axisId)
	{
		case ID_AXIS_Z:
		{
			stepPin = PIN_AXIS_Z_STEP;
			dirPin = PIN_AXIS_Z_DIR;
			maxSpd = configMulti.maxSpd_Z;
			accel = configMulti.accel_Z;
			speedPercent = configMulti.speedPercent_Z;
		...
		case ID_AXIS_X:
		{
			stepPin = PIN_AXIS_X_STEP;
			dirPin = PIN_AXIS_X_DIR;
			maxSpd = configMulti.maxSpd_X;
			accel = configMulti.accel_X;
			speedPercent = configMulti.speedPercent_X;
		...
		case ID_AXIS_3:
		{
			stepPin = PIN_AXIS_3_STEP;
			dirPin = PIN_AXIS_3_DIR;
			maxSpd = configMulti.maxSpd_M3;
			accel = configMulti.accel_M3;
			speedPercent = configMulti.speedPercent_M3;
			currentSpeedPercent = speedPercent * .01;
		...
		case ID_AXIS_4:
		{
			stepPin = PIN_AXIS_4_STEP;
			dirPin = PIN_AXIS_4_DIR;
			maxSpd = configMulti.maxSpd_M4;
			accel = configMulti.accel_M4;
			speedPercent = configMulti.speedPercent_M4;
			currentSpeedPercent = speedPercent * .01;
	...
	Serial.print("maxSpeedAndDirection");
	...
	if (configMulti.axisId == ID_SPINDLE)
	{
		if (configMain.synchro_M3_Spindle == 1)//Synchronized M3 and Spindle
		{
			...
			if (speed_M3 > configMain.maxSpd_M3)
			{
				maxSpd = maxSpd / (configMain.synchro_M3_Percentage * .01);
				speed_M3 = configMain.maxSpd_M3 * direction;
			}
			...
			switch (incomingByte)
			{
				...
				case 111:  // Axis Speed
				{
					...
					switch (configMulti.axisId)
					{
						case ID_AXIS_Z:
						{
							configMulti.speedPercent_Z = newSpeedPercent;
							currentSpeedPercent = float((newSpeedPercent) * .01);// *direction);
							break;
						}
						case ID_AXIS_X:
...
//	const char * axis_Char = "Axis-";
	...
	switch (configMain.axisId)
	{
		case ID_AXIS_Z:
		{
			speedPercentAxis = configMain.speedPercent_Z;
			stepPin = PIN_AXIS_Z_STEP;
			dirPin = PIN_AXIS_Z_DIR;
		...
		case ID_AXIS_X:
		{
			...
			speedPercentAxis = configMain.speedPercent_X;
			stepPin = PIN_AXIS_X_STEP;
			dirPin = PIN_AXIS_X_DIR;
		...
		case ID_AXIS_3:
		{
			speedPercentAxis = configMain.speedPercent_M3;
			stepPin = PIN_AXIS_3_STEP;
			dirPin = PIN_AXIS_3_DIR;
		...
		case ID_AXIS_4:
		{
			speedPercentAxis = configMain.speedPercent_M4;
			stepPin = PIN_AXIS_4_STEP;
			dirPin = PIN_AXIS_4_DIR;
	...
	if (stepper_Axis_Go)
	{
		...
		switch (configMain.axisId)
		{
			case ID_AXIS_Z:
			{
				...
				MilliDelay(10);
				rotateController_MainAxis.overrideSpeed(configMain.speedPercent_Z * .01);
				break;
			}
			case ID_AXIS_X:
			{
				...
				MilliDelay(10);
				rotateController_MainAxis.overrideSpeed(configMain.speedPercent_X * .01);
				break;
			}
			case ID_AXIS_3:
			{
				...
				rotateController_MainAxis.overrideSpeed(0);
				MilliDelay(10);
				rotateController_MainAxis.overrideSpeed(configMain.speedPercent_M3 * .01);
				break;
			}
			case ID_AXIS_4:
			{
				...
				rotateController_MainAxis.overrideSpeed(0);
				MilliDelay(10);
				rotateController_MainAxis.overrideSpeed(configMain.speedPercent_M4 * .01);
				break;
			}
		}
	}
	...
	while (stepper_Axis_Go || stepper_Spindle_Go)
	{
		...
		if (SerialAvailable() >= 0)
		{
			...
			switch (incomingByte)
			{
				...
				{
					Serial.print("Keep Enabled-Spindle: ");
					Serial.println(configSetup.keepSpindleEnabled);
				...
				case 90: // Z - Z Axis CW
				{
					if (!rotateController_MainAxis.isRunning())
					{
						...
						stepper_Z.setMaxSpeed(configMain.maxSpd_Z* direction_Axis);
				...
				case 99: // c - Axis stop
				{
					Serial.print ("Keep Enabled-Z: ");
					Serial.println(configSetup.keepZEnabled);
					Serial.print("Keep Enabled-X: ");
					Serial.println(configSetup.keepXEnabled);
					Serial.print("Keep Enabled-M3: ");
					Serial.println(configSetup.keepM3Enabled);
					Serial.print("Keep Enabled-M4: ");
					Serial.println(configSetup.keepM4Enabled);
					...
					switch (configMain.axisId)
					{
						...
						{
							SetEnable(ID_AXIS_Z, false, true);
							keepMotorEnabled = configSetup.keepZEnabled;
							break;
						}
						case ID_AXIS_X:
						{
							SetEnable(ID_AXIS_X, false, true);
							keepMotorEnabled = configSetup.keepXEnabled;
							break;
						}
						case ID_AXIS_3:
						{
							SetEnable(ID_AXIS_3, false, true);
							keepMotorEnabled = configSetup.keepM3Enabled;
							break;
						}
						case ID_AXIS_4:
						{
							SetEnable(ID_AXIS_4, false, true);
							Serial.println("SetEnable(ID_AXIS_4, false, true)");
							keepMotorEnabled = configSetup.keepM4Enabled;
							Serial.print("keepMotorEnabled:");
							Serial.println(keepMotorEnabled);
					...
					if (configSetup.keepSteppersEnabled == 1)
					{
						...
						SerialPrint("\xFF\xFF\xFF");
						Serial.println("11..........pageMain.bt11.val=1");

					}
					else
					{
						if (keepMotorEnabled == 0)
						{
							...
							SerialPrint("Disabled");
							SerialPrint("\x22\xFF\xFF\xFF");
							Serial.println("12............pageMain.bt11.val=0");
						}
						else
						{
							Serial.println("13...............keepMotorEnabled=1");
						}
					}
				...
				case 165: // ¥ - X Axis CCW
				{
					if (!rotateController_MainAxis.isRunning())
					{
						...
						stepper_X.setMaxSpeed(configMain.maxSpd_X* direction_Axis);
				...
				case 171: // « - M3 Axis CCW
				{
					if (!rotateController_MainAxis.isRunning())
					{
						...
						stepper_M3.setMaxSpeed(configMain.maxSpd_M3* direction_Axis);
				...
				case 209: // « - M4 Axis CCW
				{
					if (!rotateController_MainAxis.isRunning())
					{
						...
						if (configSetup.radialOrLinear_M4 == 1) // Linear
						{
							direction_Axis = DIR_CW;
						}

						speedPercentAxis = (float)((configMain.speedPercent_M4) * .01);
						stepper_M4.setMaxSpeed(configMain.maxSpd_M4* direction_Axis);
...
//	int direction = 0;
	...
	int waveCount = 1;
	int waveMotorId = 1;
	bool stepper_1_ChangeDir = false;
	bool stepper_2_ChangeDir = false;
	bool stepper_3_ChangeDir = false;
	bool stepper_4_ChangeDir = false;
	bool stepper_5_ChangeDir = false;

	checkedCount = 0;

	if (configMulti.waves_Z > 0 || configMulti.waves_X > 0 || configMulti.waves_M3 > 0 || configMulti.waves_M4 > 0 || configMulti.waves_Spindle > 0)
	{
		doWaves = true;
	}

	Serial.print("1.waveCount: ");
	...
	if (multiAxisChecked == 1)
	{
		...
		stepsToMove_1 = (configMulti.targetDistance_Z / (configSetup.distancePerRev_Z) * (configSetup.steps360_Z * configSetup.microsteps_Z));
		...
		Serial.print("1.configMulti.waves_Z: ");
		Serial.println(configMulti.waves_Z);

		if (doWaves && configMulti.waves_Z > 0)
		{
			//stepsToMove_1 = stepsToMove_1 / (configMulti.waves_Z * 2);
			waveMotorId = 1;
			waveCount = configMulti.waves_Z *2;
			stepper_1_ChangeDir = true;

			Serial.print("waveMotorId: ");
			Serial.println(waveMotorId);
			Serial.print("2.configMulti.waves_Z: ");
			Serial.println(configMulti.waves_Z);
			Serial.print("2.waveCount: ");
			Serial.println(waveCount);
		}
		else if (doWaves && configMulti.waves_Z < 0)
		{
			stepper_1_ChangeDir = true;
			Serial.print("3.configMulti.waves_Z: ");
			Serial.println(configMulti.waves_Z);
		}
	...
	if (multiAxisChecked == 2)
	{
		...
		if (checkedCount == 1)
		{
			...
			stepsToMove_1 = (configMulti.targetDistance_X / (configSetup.distancePerRev_X) * (configSetup.steps360_X * configSetup.microsteps_X)) * direction_1;
			...
			if (doWaves && configMulti.waves_X > 0)
			{
				//stepsToMove_1 = stepsToMove_1 / (configMulti.waves_X * 2);
				waveMotorId = 2;
				waveCount = configMulti.waves_X * 2;
				stepper_1_ChangeDir = true;
				Serial.print("3.waveCount: ");
				Serial.println(waveCount);
			}
			else if (doWaves && configMulti.waves_X < 0)
			{
				stepper_1_ChangeDir = true;
			}
		...
		else // checkedCount = 2
		{
			...
			stepsToMove_2 = static_cast<int32_t>(configMulti.targetDistance_X / (configSetup.distancePerRev_X) * (configSetup.steps360_X * configSetup.microsteps_X)) * direction_2;
			...
			if (doWaves && configMulti.waves_X > 0)
			{
				//stepsToMove_2 = stepsToMove_2 / (configMulti.waves_X * 2);
				waveMotorId = 2;
				waveCount = configMulti.waves_X * 2;
				stepper_2_ChangeDir = true;
				Serial.print("4.waveCount: ");
				Serial.println(waveCount);
			}
			else if (doWaves && configMulti.waves_X < 0)
			{
	...
	if (multiAxisChecked == 4)
	{
		...
		if (checkedCount == 1)
		{
			...
			if (configSetup.radialOrLinear_M3 == RADIAL_M3)
			{
				stepsPerRevolution = (int)round(configSetup.microsteps_M3 * configSetup.steps360_M3 * configSetup.gearRatio_M3);
			...
			else
			{
				...
				stepsToMove_1 = (configMulti.targetDistance_M3 / (configSetup.distancePerRev_M3) * (configSetup.steps360_M3 * configSetup.microsteps_M3)) * direction_1;
			}

			newSpeed_1 = configMulti.speedPercent_M3 * configMulti.maxSpd_M3 * .01;
			accel_1 = configMulti.accel_M3;

			if (doWaves && configMulti.waves_M3 > 0)
			{
				//stepsToMove_1 = stepsToMove_1 / (configMulti.waves_M3 * 2);
				waveMotorId = 3;
				waveCount = configMulti.waves_M3 * 2;
				stepper_1_ChangeDir = true;
				Serial.print("5.waveCount: ");
				Serial.println(waveCount);
			}
			else if (doWaves && configMulti.waves_M3 < 0)
			{
				stepper_1_ChangeDir = true;
			}
		...
		else if (checkedCount == 2)
		{
			...
			else
			{
				...
				stepsToMove_2 = (configMulti.targetDistance_M3 / (configSetup.distancePerRev_M3) * (configSetup.steps360_M3 * configSetup.microsteps_M3)) * direction_2;
			}
			...
			if (doWaves && configMulti.waves_M3 > 0)
			{
				//stepsToMove_2 = stepsToMove_2 / (configMulti.waves_M3 * 2);
				waveMotorId = 3;
				waveCount = configMulti.waves_M3 * 2;
				stepper_2_ChangeDir = true;
				Serial.print("6.waveCount: ");
				Serial.println(waveCount);
			}
			else if (doWaves && configMulti.waves_M3 < 0)
			{
		...
		else // checkedCount = 3
		{
			...
			else
			{
				...
				stepsToMove_3 = (configMulti.targetDistance_M3 / (configSetup.distancePerRev_M3) * (configSetup.steps360_M3 * configSetup.microsteps_M3)) * direction_3;
			}

			newSpeed_3 = configMulti.speedPercent_M3 * configMulti.maxSpd_M3 * .01;
			accel_3 = configMulti.accel_M3;

			Serial.print("configMulti.waves_M3: ");
			Serial.println(configMulti.waves_M3);
			if (doWaves && configMulti.waves_M3 > 0)
			{
				//stepsToMove_3 = stepsToMove_3 / (configMulti.waves_M3 * 2);
				waveMotorId = 3;
				waveCount = configMulti.waves_M3 * 2;
				stepper_3_ChangeDir = true;
				Serial.print("7.waveCount: ");
				Serial.println(waveCount);
			}
			else if (doWaves && configMulti.waves_M3 < 0)
			{
	...
	if (multiAxisChecked == 8)
	{
		...
		if (checkedCount == 1)
		{
			...
			if (configSetup.radialOrLinear_M4 == RADIAL_M4)
			{
				stepsPerRevolution = (int)round(configSetup.microsteps_M4 * configSetup.steps360_M4 * configSetup.gearRatio_M4);
			...
			else
			{
				...
				digitalWrite(limitPin_Max_1, HIGH);
				digitalWrite(limitPin_Min_1, HIGH);
				stepsToMove_1 = (configMulti.targetDistance_M4 / (configSetup.distancePerRev_M4) * (configSetup.steps360_M4 * configSetup.microsteps_M4)) * direction_1;
			}
			newSpeed_1 = configMulti.speedPercent_M4 * configMulti.maxSpd_M4 * .01;
			accel_1 = configMulti.accel_M4;

			if (doWaves && configMulti.waves_M4 > 0)
			{
				//stepsToMove_1 = stepsToMove_1 / (configMulti.waves_M4 * 2);
				waveMotorId = 4;
				waveCount = configMulti.waves_M4 * 2;
				stepper_1_ChangeDir = true;
				Serial.print("8.waveCount: ");
				Serial.println(waveCount);
			}
			else if (doWaves && configMulti.waves_M4 < 0)
			{
				stepper_1_ChangeDir = true;
			}
		...
		else if (checkedCount == 2)
		{
			...
			else
			{
				...
				stepsToMove_2 = static_cast<int32_t>(configMulti.targetDistance_M4 / (configSetup.distancePerRev_M4) * (configSetup.steps360_M4 * configSetup.microsteps_M4)) * direction_2;
			}
			...
			if (doWaves && configMulti.waves_M4 > 0)
			{
				//stepsToMove_2 = stepsToMove_2 / (configMulti.waves_M4 * 2);
				waveMotorId = 4;
				waveCount = configMulti.waves_M4 * 2;
				stepper_2_ChangeDir = true;
				Serial.print("9.waveCount: ");
				Serial.println(waveCount);
			}
			else if (doWaves && configMulti.waves_M4 < 0)
			{
		...
		else if (checkedCount == 3)
		{
			...
			else
			{
				...
				stepsToMove_3 = (configMulti.targetDistance_M4 / (configSetup.distancePerRev_M4) * (configSetup.steps360_M4 * configSetup.microsteps_M4)) * direction_3;
			}
			...
			if (doWaves && configMulti.waves_M4 > 0)
			{
				//stepsToMove_3 = stepsToMove_3 / (configMulti.waves_M4 * 2);
				waveMotorId = 4;
				waveCount = configMulti.waves_M4 * 2;
				stepper_3_ChangeDir = true;
				Serial.print("10.waveCount: ");
				Serial.println(waveCount);
			}
			else if (doWaves && configMulti.waves_M4 < 0)
			{
		...
		else //(checkedCount == 4)
		{
			...
			else
			{
				...
				stepsToMove_4 = (configMulti.targetDistance_M4 / (configSetup.distancePerRev_M4) * (configSetup.steps360_M4 * configSetup.microsteps_M4)) * direction_4;
			}
			...
			if (doWaves && configMulti.waves_M4 > 0)
			{
				//stepsToMove_4 = stepsToMove_4 / (configMulti.waves_M4 * 2);
				waveMotorId = 4;
				waveCount = configMulti.waves_M4 * 2;
				stepper_4_ChangeDir = true;
				Serial.print("11.waveCount: ");
				Serial.println(waveCount);
			}
			else if (doWaves && configMulti.waves_M4 < 0)
			{
	...
	if (multiAxisChecked == 16)
	{
		...
		if (checkedCount == 1)
		{
			...
			if (doWaves && configMulti.waves_Spindle > 0)
			{
				//stepsToMove_1 = stepsToMove_1 / (configMulti.waves_Spindle * 2);
				waveMotorId = 5;
				waveCount = configMulti.waves_Spindle * 2;
				stepper_1_ChangeDir = true;
				Serial.print("12.waveCount: ");
				Serial.println(waveCount);
			}
			else if (doWaves && configMulti.waves_Spindle < 0)
			{
				stepper_1_ChangeDir = true;
			}
		...
		else if (checkedCount == 2)
		{
			...
			if (doWaves && configMulti.waves_Spindle > 0)
			{
				//stepsToMove_2 = stepsToMove_2 / (configMulti.waves_Spindle * 2);
				waveMotorId = 5;
				waveCount = configMulti.waves_Spindle * 2;
				stepper_2_ChangeDir = true;
				Serial.print("13.waveCount: ");
				Serial.println(waveCount);
			}
			else if (doWaves && configMulti.waves_Spindle < 0)
			{
		...
		else if (checkedCount == 3)
		{
			...
			if (doWaves && configMulti.waves_Spindle > 0)
			{
				//stepsToMove_3 = stepsToMove_3 / (configMulti.waves_Spindle * 2);
				waveMotorId = 5;
				waveCount = configMulti.waves_Spindle * 2;
				stepper_3_ChangeDir = true;
				Serial.print("14.waveCount: ");
				Serial.println(waveCount);
			}
			else if (doWaves && configMulti.waves_Spindle < 0)
			{
		...
		else if (checkedCount == 4)
		{
			...
			if (doWaves && configMulti.waves_Spindle > 0)
			{
				//stepsToMove_4 = stepsToMove_4 / (configMulti.waves_Spindle * 2);
				waveMotorId = 5;
				waveCount = configMulti.waves_Spindle * 2;
				stepper_4_ChangeDir = true;
				Serial.print("15.waveCount: ");
				Serial.println(waveCount);
			}
			else if (doWaves && configMulti.waves_Spindle < 0)
			{
		...
		else //(checkedCount == 5)
		{
			...
			if (doWaves && configMulti.waves_Spindle > 0)
			{
				//stepsToMove_5 = stepsToMove_5 / (configMulti.waves_Spindle * 2);
				waveMotorId = 5;
				waveCount = configMulti.waves_Spindle * 2;
				stepper_5_ChangeDir = true;
				Serial.print("16.waveCount: ");
				Serial.println(waveCount);
			}
			else if (doWaves && configMulti.waves_Spindle < 0)
			{
	...
	{
		stepsToMove_1 = stepsToMove_1 / waveCount;
		stepsToMove_2 = stepsToMove_2 / waveCount;
		stepsToMove_3 = stepsToMove_3 / waveCount;
		stepsToMove_4 = stepsToMove_4 / waveCount;
		stepsToMove_5 = stepsToMove_5 / waveCount;

		stepper1.setTargetRel(stepsToMove_1);
		stepper2.setTargetRel(stepsToMove_2);
		stepper3.setTargetRel(stepsToMove_3);
		stepper4.setTargetRel(stepsToMove_4);
		stepper5.setTargetRel(stepsToMove_5);
	}
	Serial.print("17.waveCount: ");
	Serial.println(waveCount);
	...
	for (int i = 1; i <= waveCount; i++)
	{
		...
		if (i > 1)
		{
			...
			if (stepper_1_ChangeDir)
			{
				stepper1.setTargetRel(stepsToMove_1 *= -1);
				Serial.print("stepsToMove_1: ");
				Serial.println(stepsToMove_1);
			}
			if (stepper_2_ChangeDir)
			{
				stepper2.setTargetRel(stepsToMove_2 *= -1);
			}
			if (stepper_3_ChangeDir)
			{
				stepper3.setTargetRel(stepsToMove_3 *= -1);
			}
			if (stepper_4_ChangeDir)
			{
				stepper4.setTargetRel(stepsToMove_4 *= -1);
			}
			if (stepper_5_ChangeDir)
			{
				stepper5.setTargetRel(stepsToMove_5 *= -1);
			}
		}
	...
	switch (configIndex_Prime.axisId)
	{
		case ID_SPINDLE:
		{
			...
			if (configIndex_Prime.synchro_M3_Spindle == 1)//Synchronized M3 and Spindle
			{
				...
				stepperAxis_M3.setAcceleration(configIndex_Prime.accel_M3);
		...
		case ID_AXIS_3:
		{
			//stepsToMove = 0;
			SetEnable(ID_AXIS_3, true, true);
			stepsPerRevolution_M3 = (int)round(configSetup.microsteps_M3 * configSetup.steps360_M3 * configSetup.gearRatio_M3);
			//maxSpeed
		...
		case ID_AXIS_4:
		{
			//stepsToMove = 0;
			SetEnable(ID_AXIS_4, true, true);
			stepsPerRevolution_M4 = (int)round(configSetup.microsteps_M4 * configSetup.steps360_M4 * configSetup.gearRatio_M4);
			//maxSpeed
...
void GreekKeyPattern_Initial(int segmentCount)
{
...
#ifdef DEBUG
	Serial.print("revolutions_Spindle: ");
	Serial.println(revolutions_Spindle);
	Serial.print("configGreekKey.countPatternPer360_Pattern: ");
	...
	switch (configGreekKey.axisId)
	{
		case ID_AXIS_Z: // Z Axis
		{
			distance_Axis = configGreekKey.segmentLength_Pattern / configSetup.distancePerRev_Z;
			axisShortLegSteps = (int)round(distance_Axis * configSetup.steps360_Z * configSetup.microsteps_Z);
			break;
		}
		case ID_AXIS_X: // X Axis
		{
			if (configSetup.xAltX == 0)
			{
				distance_Axis = configGreekKey.segmentLength_Pattern / configSetup.distancePerRev_X;
				axisShortLegSteps = (int)round(distance_Axis * configSetup.steps360_X * configSetup.microsteps_X);
			}
			else
			{
				distance_Axis = configGreekKey.segmentLength_Pattern / configSetup.distancePerRev_XAlt;
				axisShortLegSteps = (int)round(distance_Axis * configSetup.steps360_XAlt * configSetup.microsteps_XAlt);
			}
...
bool GreekKey_Move_Axis(float segmentSteps, float multiplier, int direction, bool updatePosition)
{
	...
	Serial.println(multiplier * segmentSteps);
	Serial.println("=======================");
#endif // VERBOSE
...
#ifdef DEBUG
	Serial.print("Axis position: ");
	Serial.println(stepperAxis.getPosition());
#endif // DEBUG
	return retVal;
}
...
bool GreekKey_Angular_TwoAxes(int direction)
{
	...
	switch (primaryAxisId)
	{
		case ID_AXIS_Z:
		{
			...
			shortLegLength_Primary = (int32_t)(((configGreekKey.segmentLength_File / configSetup.distancePerRev_Z) * (configSetup.steps360_Z * configSetup.microsteps_Z)));
			SetEnable(ID_AXIS_Z, true, true);
			break;
		}
		case ID_AXIS_X:
		{
			...
			currentSpeedPercent_Primary = speedPercent_Primary * .01;
			shortLegLength_Primary = (int32_t)(((configGreekKey.segmentLength_File / configSetup.distancePerRev_X) * (configSetup.steps360_X * configSetup.microsteps_X)));
		...
		case ID_AXIS_3:
		{
			...
			currentSpeedPercent_Primary = speedPercent_Primary * .01;
			shortLegLength_Primary = (int32_t)(((configGreekKey.segmentLength_File / configSetup.distancePerRev_M3) * (configSetup.steps360_M3 * configSetup.microsteps_M3)));
			switch (configSetup.radialOrLinear_M3)
			{
				case RADIAL_M3:
				{
		...
		case ID_AXIS_4:
		{
			...
			switch (configSetup.radialOrLinear_M3)
			{
				...
				case LINEAR_M3:
				{
					shortLegLength_Primary = (int32_t)(((configGreekKey.segmentLength_File / configSetup.distancePerRev_M4) * (configSetup.steps360_M4 * configSetup.microsteps_M4)));
					break;
				}
			}
	...
	switch (secondaryAxisId)
	{
		case ID_AXIS_Z:
		{
			...
			shortLegLength_Secondary = (int32_t)(((configGreekKey.segmentLength_File / configSetup.distancePerRev_Z) * (configSetup.steps360_Z * configSetup.microsteps_Z)));
			SetEnable(ID_AXIS_Z, true, true);
			break;
		}
		case ID_AXIS_X:
		{
			...
			currentSpeedPercent_Secondary = speedPercent_Secondary * .01;
			shortLegLength_Secondary = (int32_t)(((configGreekKey.segmentLength_File / configSetup.distancePerRev_X) * (configSetup.steps360_X * configSetup.microsteps_X)));
		...
		case ID_AXIS_3:
		{
			...
			currentSpeedPercent_Secondary = speedPercent_Secondary * .01;
			shortLegLength_Secondary = (int32_t)(((configGreekKey.segmentLength_File / configSetup.distancePerRev_M3) * (configSetup.steps360_M3 * configSetup.microsteps_M3)));
		...
		case ID_AXIS_4:
		{
			...
			currentSpeedPercent_Secondary = speedPercent_Secondary * .01;
			shortLegLength_Secondary = (int32_t)(((configGreekKey.segmentLength_File / configSetup.distancePerRev_M4) * (configSetup.steps360_M4 * configSetup.microsteps_M4)));
...
bool GreekKey_Move_Spindle(float segmentSteps, float multiplier, int direction)
{
	...
	Serial.print("targetStepsRounded: ");
	Serial.println(targetStepsRounded);
	Serial.print("stepper_1 position: ");
...
EndSegmentCount:
	...
	shortSegmentStepsAxisZ = (int)(((configGreekKey.segmentLength_File / configSetup.distancePerRev_Z) * (configSetup.steps360_Z * configSetup.microsteps_Z)));
#ifdef VERBOSE
	...
	Serial.println(configSetup.distancePerRev_Z);
		
	Serial.print("configSetup.steps360_Axis_Z:");
	Serial.println(configSetup.steps360_Z);
		
	Serial.print("configSetup.microsteps_Axis_Z:");
	Serial.println(configSetup.microsteps_Z);
	...
	if (configSetup.xAltX == 0)
	{
		shortSegmentStepsAxisX = (int)(((configGreekKey.segmentLength_File / configSetup.distancePerRev_X) * (configSetup.steps360_X * configSetup.microsteps_X)));
	}
	else
	{
		shortSegmentStepsAxisX = (int)(((configGreekKey.segmentLength_File / configSetup.distancePerRev_XAlt) * (configSetup.steps360_XAlt * configSetup.microsteps_XAlt)));
	}
	...
	switch (configSetup.radialOrLinear_M3)
	{
		case RADIAL_M3:
		{
			...
			{
				stepsPer360 = configSetup.steps360_M3 * configSetup.microsteps_M3 * configSetup.gearRatio_M3;
	...
	switch (configSetup.radialOrLinear_M4)
	{
		case RADIAL_M4:
		{
			...
			{
				stepsPer360 = configSetup.steps360_M4 * configSetup.microsteps_M4 * configSetup.gearRatio_M4;
	...
	{ 
		...
		for (int i = 0; i <= fileLineCount; i++)
		{
			...
			switch (moveType)
			{
					...
					switch (configGreekKey.radialOrAxial_File)
					{
						case RADIAL: // Axis: Negative move direction is CCW
						{
							...
							else //3: Actual
							{
								...
								case ID_AXIS_Z: // Z Axis
								{
									axisSteps = round((segmentMultiplier / configSetup.distancePerRev_Z) * (configSetup.steps360_Z * configSetup.microsteps_Z));
									break;
								}
								case ID_AXIS_X: // X Axis
								{
									if (configSetup.xAltX == 0)
									{
										axisSteps = round((segmentMultiplier / configSetup.distancePerRev_X) * (configSetup.steps360_X * configSetup.microsteps_X));
									}
									else
									{
										axisSteps = round((segmentMultiplier / configSetup.distancePerRev_XAlt) * (configSetup.steps360_XAlt * configSetup.microsteps_XAlt));
									}
									break;
								}
								case ID_AXIS_3: // M3 Axis
						...
						case RADIAL: // 
						{
							...
							else  // Actual
							{
								switch (configGreekKey.axisId)
								{
									case ID_AXIS_Z: // Z Axis
									{
										axisSteps = (int)(round((hv_AxisLegLength / configSetup.distancePerRev_Z) * (configSetup.steps360_Z * configSetup.microsteps_Z)));
										break;
									}
									case ID_AXIS_X: // X Axis
									{
										if (configSetup.xAltX == 0)
										{
											axisSteps = (int)(round((hv_AxisLegLength / configSetup.distancePerRev_X) * (configSetup.steps360_X * configSetup.microsteps_X)));
										}
										else
										{
											axisSteps = (int)(round((hv_AxisLegLength / configSetup.distancePerRev_XAlt) * (configSetup.steps360_XAlt * configSetup.microsteps_XAlt)));
										}
										break;
									}
									case ID_AXIS_3: // M3 Axis
						...
						case AXIAL: // Switch spindle and axis
						{
							...
							else  // Actual
							{
								switch (configGreekKey.axisId)
								{
									case ID_AXIS_Z: // Z Axis
									{
										axisSteps = round((hv_SpindleLegLength / configSetup.distancePerRev_Z) * (configSetup.steps360_Z * configSetup.microsteps_Z));
										break;
									}
									case ID_AXIS_X: // X Axis
									{
										if (configSetup.xAltX == 0)
										{
											axisSteps = round((hv_SpindleLegLength / configSetup.distancePerRev_X) * (configSetup.steps360_X * configSetup.microsteps_X));
										}
										else
										{
											axisSteps = round((hv_SpindleLegLength / configSetup.distancePerRev_XAlt) * (configSetup.steps360_XAlt * configSetup.microsteps_XAlt));
										}
										break;
									}
									case ID_AXIS_3: // M3 Axis
				...
				case 81: //Q - MultiSync page 
				{
					Serial.print("^^^^Q: ");
					Serial.println(commandMultiSync);
					...
					Serial.print("++++++++++++commandMultiSync: ");
					Serial.println(commandMultiSync);

					SerialPrint("wepo ");

					if (commandMultiSync == "Q")
					{
						Serial.println("Execute MultiSync page");

						Multi_Sync();
					}
					else if (commandMultiSync == "A")
					{
						// X target
						configMulti.targetDistance_X = segmentMultiplier;
						EEPROM.put(eePromAddress_Multi, configMulti);
					...
					else if (commandMultiSync == "B")
					{
						// X waves
						configMulti.waves_X = segmentMultiplier;
#ifdef DEBUG
						Serial.print("configMulti.waves_X: ");
						Serial.println(configMulti.waves_X);
#endif // DEBUG
						EEPROM.put(eePromAddress_Multi, configMulti);
					...
					else if (commandMultiSync == "C")
					{
						//  X direction
						if (segmentMultiplier == 3)
						{
								direction = (-1);
						}
						else
						...
						dirInt = static_cast<int>(segmentMultiplier);
						SerialPrint(dirInt);
						eePromAddress = 720;
#ifdef DEBUG
						Serial.print("X ");
					...
					else if (commandMultiSync == "D")
					{
						...
						configMulti.checked_X = segmentMultiplier;
						configMulti.multiAxesIDs = configMulti.checked_Z + configMulti.checked_X + configMulti.checked_M3 + configMulti.checked_M4 + configMulti.checked_Spindle;

						//////  X checked
						////multiAxisChecked = 2 & configMulti.multiAxesIDs; // X
						////if (multiAxisChecked == 2) // X currently checked
						////{
						////	if (segmentMultiplier != 1) // Uncheck X
						////	{
						////		configMulti.multiAxesIDs = configMulti.multiAxesIDs-2;
						////	}

						////}
						////else // X currently unchecked
						////{
						////	if (segmentMultiplier == 1) // Check X
						////	{
						////		configMulti.multiAxesIDs = configMulti.multiAxesIDs+2;
						////	}
						...
						///*value0 = configMulti.speedPercent_Spindle;
						//value8 = configMulti.axisId;
						//value16 = configMulti.multiAxesIDs;
						//value24 = 0;
						//SendPackedData(eePromAddress_Nextion);*/

					}
					else if (commandMultiSync == "E")
					{
						// Z target
						configMulti.targetDistance_Z = segmentMultiplier;
						EEPROM.put(eePromAddress_Multi, configMulti);
					...
					else if (commandMultiSync == "F")
					{
						// Z waves
						configMulti.waves_Z = segmentMultiplier;
#ifdef DEBUG
						Serial.print("configMulti.waves_Z: ");
						Serial.println(configMulti.waves_Z);
#endif // DEBUG
						EEPROM.put(eePromAddress_Multi, configMulti);
					...
					else if (commandMultiSync == "H")
					{
						...
						////multiAxisChecked = 1 & configMulti.multiAxesIDs; // Z
						////if (multiAxisChecked == 1) // Z currently checked
						////{
						////	if (segmentMultiplier != 1) // Uncheck Z
						////	{
						////		configMulti.multiAxesIDs = configMulti.multiAxesIDs - 1;
						////	}

						////}
						////else // Z currently unchecked
						////{
						////	if (segmentMultiplier == 1) // Check Z
						////	{
						////		configMulti.multiAxesIDs = configMulti.multiAxesIDs + 1;
						////	}
						...
						configMulti.checked_Z = segmentMultiplier;
						configMulti.multiAxesIDs = configMulti.checked_Z + configMulti.checked_X + configMulti.checked_M3 + configMulti.checked_M4 + configMulti.checked_Spindle;
					...
					else if (commandMultiSync == "I")
					{
						// M3 target
						configMulti.targetDistance_M3 = segmentMultiplier;
						EEPROM.put(eePromAddress_Multi, configMulti);
					...
					else if (commandMultiSync == "J")
					{
						// M3 waves
						configMulti.waves_M3 = segmentMultiplier;
#ifdef DEBUG
						Serial.print("configMulti.waves_M3: ");
						Serial.println(configMulti.waves_M3);
#endif // DEBUG
					...
					else if (commandMultiSync == "L")
					{
						...
						////multiAxisChecked = 4 & configMulti.multiAxesIDs; // M3
						////if (multiAxisChecked == 4) // M3 currently checked
						////{
						////	if (segmentMultiplier != 1) // Uncheck M3
						////	{
						////		configMulti.multiAxesIDs = configMulti.multiAxesIDs - 4;
						////	}

						////}
						////else // M3 currently unchecked
						////{
						////	if (segmentMultiplier == 1) // Check M3
						////	{
						////		configMulti.multiAxesIDs = configMulti.multiAxesIDs + 4;
						////	}
						...
						configMulti.checked_M3 = segmentMultiplier;
						configMulti.multiAxesIDs = configMulti.checked_Z + configMulti.checked_X + configMulti.checked_M3 + configMulti.checked_M4 + configMulti.checked_Spindle;
					...
					else if (commandMultiSync == "M")
					{
						// M4 target
						configMulti.targetDistance_M4 = segmentMultiplier;
						EEPROM.put(eePromAddress_Multi, configMulti);
					...
					else if (commandMultiSync == "N")
					{
						// M4 waves
						configMulti.waves_M4 = segmentMultiplier;
#ifdef DEBUG
						Serial.print("configMulti.waves_M4: ");
						Serial.println(configMulti.waves_M4);
#endif // DEBUG
						EEPROM.put(eePromAddress_Multi, configMulti);
					...
					else if (commandMultiSync == "P")
					{
						...
						////multiAxisChecked = 8 & configMulti.multiAxesIDs; // M4
						////if (multiAxisChecked == 8) // M4 currently checked
						////{
						////	if (segmentMultiplier != 1) // Uncheck M4
						////	{
						////		configMulti.multiAxesIDs = configMulti.multiAxesIDs - 8;
						////	}

						////}
						////else // M4 currently unchecked
						////{
						////	if (segmentMultiplier == 1) // Check M4
						////	{
						////		configMulti.multiAxesIDs = configMulti.multiAxesIDs + 8;
						////	}
						...
						configMulti.checked_M4 = segmentMultiplier;
						configMulti.multiAxesIDs = configMulti.checked_Z + configMulti.checked_X + configMulti.checked_M3 + configMulti.checked_M4 + configMulti.checked_Spindle;
					...
					else if (commandMultiSync == "R")
					{
						// Run MultiSync return
						ReturnToStartPosition_Multi();

					}
					else if (commandMultiSync == "S")
					{
						// Spindle target
						configMulti.targetDistance_Spindle = segmentMultiplier;
#ifdef DEBUG
					...
					else if (commandMultiSync == "T")
					{
						// Spindle waves
						configMulti.waves_Spindle = segmentMultiplier;
#ifdef DEBUG
						Serial.print("configMulti.waves_Spindle: ");
						Serial.println(configMulti.waves_Spindle);
#endif // DEBUG
						EEPROM.put(eePromAddress_Multi, configMulti);
					...
					else if (commandMultiSync == "V")
					{
						...
						////multiAxisChecked = 16 & configMulti.multiAxesIDs; // Spindle
						////if (multiAxisChecked == 16) // Spindle currently checked
						////{
						////	if (segmentMultiplier != 1) // Uncheck Spindle
						////	{
						////		configMulti.multiAxesIDs = configMulti.multiAxesIDs - 16;
						////	}

						////}
						////else // Spindle currently unchecked
						////{
						////	if (segmentMultiplier == 1) // Check Spindle
						////	{
						////		configMulti.multiAxesIDs = configMulti.multiAxesIDs + 16;
						////	}
						...
						configMulti.checked_Spindle = segmentMultiplier;
						configMulti.multiAxesIDs = configMulti.checked_Z + configMulti.checked_X + configMulti.checked_M3 + configMulti.checked_M4 + configMulti.checked_Spindle;
				...
				case 82: // R - Rose page command
				{
					...
					{
						configRose.n = segmentMultiplier;
						SerialWrite(0x22);
						SerialPrint(segmentMultiplier);
						SerialWrite(0x22);
						eePromAddress = 644;
					...
					{
						configRose.d = segmentMultiplier;
						SerialWrite(0x22);
						SerialPrint(segmentMultiplier);
						SerialWrite(0x22);
						eePromAddress = 468;
					...
					{
						configRose.spindleRevolutions = segmentMultiplier;
						SerialWrite(0x22);
						SerialPrint(segmentMultiplier);
						SerialWrite(0x22);
						eePromAddress = 600;
					...
					else if (commandRose == "A") // Set axis distance
					{
						...
						switch (configRose.axisId)
						{
							case ID_AXIS_Z:
							{
								configRose.amplitude_Z = segmentMultiplier;
								SerialPrint(segmentMultiplier);
								SerialWrite(0x22);
								eePromAddress = 276;
								break;
							...
							case ID_AXIS_X:
							{
								configRose.amplitude_X = segmentMultiplier;
								SerialPrint(segmentMultiplier);
								SerialWrite(0x22);
								eePromAddress = 284;
								break;
							...
							case ID_AXIS_3:
							{
								configRose.amplitude_M3 = segmentMultiplier;
								SerialPrint(segmentMultiplier);
								SerialWrite(0x22);
								eePromAddress = 520;
								break;
							...
							case ID_AXIS_4:
							{
								configRose.amplitude_M4 = segmentMultiplier;
								SerialPrint(segmentMultiplier);
								SerialWrite(0x22);
								eePromAddress = 68;
								break;
...
/// <returns></returns>
void SerialPrint(String text, int decimalPlaces)
{
	switch (serialId)
	{
		case 0:
...
/// <returns></returns>
void SerialPrint(float number, int decimalPlaces)
{
...
void GetIniFileListFromSD(int fileIndex)
{
	...
	for (int iCounter = 0; iCounter < fileIndex + 7; iCounter++)
	{
		...
		else
		{
			...
			Serial.println(fileFullName);
			Serial.print("   IndexOf .ini:  ");
			Serial.println(fileFullName.indexOf(iniExtension));
#endif // VERBOSE


			if(fileFullName.indexOf(iniExtension) >= 0 || fileFullName.indexOf(iniExtensionCaps) >= 0)
			{ 
...
//		case 81: // Q
		...
		case 81:// Q - MultiSync page settings
		{
			commandMultiSync = newSizeString[1];
...
void ReturnToStartPosition_Multi()
{
	...
	switch (stepperId_1)
	{
		case ID_AXIS_Z:
		{
			maxSpeed_1 = configSetup.maxSpd_Return_Z;
			accel_1 = configSetup.accel_Return_Z;
			break;
		}
		case ID_AXIS_X:
		{
			maxSpeed_1 = configSetup.maxSpd_Return_X;
			accel_1 = configSetup.accel_Return_X;
			break;
		}
		case ID_AXIS_3:
		{
			maxSpeed_1 = configSetup.maxSpd_Return_M3;
			accel_1 = configSetup.accel_Return_M3;
			break;
		}
		case ID_AXIS_4:
		{
			maxSpeed_1 = configSetup.maxSpd_Return_M4;
			accel_1 = configSetup.accel_Return_M4;
			break;
		}
		case ID_SPINDLE:
		{
	...
	switch (stepperId_2)
	{
		case ID_AXIS_Z:
		{
			maxSpeed_2 = configSetup.maxSpd_Return_Z;
			accel_2 = configSetup.accel_Return_Z;
			break;
		}
		case ID_AXIS_X:
		{
			maxSpeed_2 = configSetup.maxSpd_Return_X;
			accel_2 = configSetup.accel_Return_X;
			break;
		}
		case ID_AXIS_3:
		{
			maxSpeed_2 = configSetup.maxSpd_Return_M3;
			accel_2 = configSetup.accel_Return_M3;
			break;
		}
		case ID_AXIS_4:
		{
			maxSpeed_2 = configSetup.maxSpd_Return_M4;
			accel_2 = configSetup.accel_Return_M4;
			break;
		}
		case ID_SPINDLE:
		{
	...
	switch (stepperId_3)
	{
		case ID_AXIS_Z:
		{
			maxSpeed_3 = configSetup.maxSpd_Return_Z;
			accel_3 = configSetup.accel_Return_Z;
			break;
		}
		case ID_AXIS_X:
		{
			maxSpeed_3 = configSetup.maxSpd_Return_X;
			accel_3 = configSetup.accel_Return_X;
			break;
		}
		case ID_AXIS_3:
		{
			maxSpeed_3 = configSetup.maxSpd_Return_M3;
			accel_3 = configSetup.accel_Return_M3;
			break;
		}
		case ID_AXIS_4:
		{
			maxSpeed_3 = configSetup.maxSpd_Return_M4;
			accel_3 = configSetup.accel_Return_M4;
			break;
		}
		case ID_SPINDLE:
		{
	...
	switch (stepperId_4)
	{
		case ID_AXIS_Z:
		{
			maxSpeed_4 = configSetup.maxSpd_Return_Z;
			accel_4 = configSetup.accel_Return_Z;
			break;
		}
		case ID_AXIS_X:
		{
			maxSpeed_4 = configSetup.maxSpd_Return_X;
			accel_4 = configSetup.accel_Return_X;
			break;
		}
		case ID_AXIS_3:
		{
			maxSpeed_4 = configSetup.maxSpd_Return_M3;
			accel_4 = configSetup.accel_Return_M3;
			break;
		}
		case ID_AXIS_4:
		{
			maxSpeed_4 = configSetup.maxSpd_Return_M4;
			accel_4 = configSetup.accel_Return_M4;
			break;
		}
		case ID_SPINDLE:
		{
	...
	switch (stepperId_5)
	{
		case ID_AXIS_Z:
		{
			maxSpeed_5 = configSetup.maxSpd_Return_Z;
			accel_5 = configSetup.accel_Return_Z;
			break;
		}
		case ID_AXIS_X:
		{
			maxSpeed_5 = configSetup.maxSpd_Return_X;
			accel_5 = configSetup.accel_Return_X;
			break;
		}
		case ID_AXIS_3:
		{
			maxSpeed_5 = configSetup.maxSpd_Return_M3;
			accel_5 = configSetup.accel_Return_M3;
			break;
		}
		case ID_AXIS_4:
		{
			maxSpeed_5 = configSetup.maxSpd_Return_M4;
			accel_5 = configSetup.accel_Return_M4;
			break;
		}
		case ID_SPINDLE:
		{
...
void ReturnToStartPosition_MovePage(int axisId)
{
	...
	switch (axisId)
	{
...
#endif // DEBUG
			stepper_Z
				.setMaxSpeed(configSetup.maxSpd_Return_Z)
				.setAcceleration(configSetup.accel_Return_Z)
				.setTargetAbs(0);

	#ifdef DEBUG
			Serial.print("maxSpd_Return_Axis_Z: ");
			Serial.println(configSetup.accel_Return_Z);
	#endif // DEBUG
			SetEnable(ID_AXIS_Z, true, true);
	...
	#endif // Debug
			stepper_X
				.setMaxSpeed(configSetup.maxSpd_Return_X)
				.setAcceleration(configSetup.accel_Return_X)
				.setTargetAbs(0);
		...
		case ID_AXIS_3:
		{
			...
			MilliDelay(20);
			stepper_M3
				.setMaxSpeed(configSetup.maxSpd_Return_M3)
				.setAcceleration(configSetup.accel_Return_M3)
				.setTargetAbs(0);
		...
		case ID_AXIS_4:
		{
			...
			MilliDelay(20);
			stepper_M4
				.setMaxSpeed(configSetup.maxSpd_Return_M4)
				.setAcceleration(configSetup.accel_Return_M4)
				.setTargetAbs(0);
...
void RoseRadial(int direction)
{
	...
	if (configMain.synchro_M3_Spindle == 1)//Synchronized M3 and Spindle
	{
		...
		newMaxSpd_M3 = newMaxSpd_RoseSpindle * synchro_Ratio * (configMain.synchro_M3_Percentage * 0.01);

		stepperAxis_M3
			.setMaxSpeed(newMaxSpd_M3)
	...
	switch (configRose.axisId)
	{
		case ID_AXIS_Z:
		{
			...
			slideStepsAmplitude = direction * (((configRose.amplitude_Z / (configSetup.distancePerRev_Z)) * configSetup.steps360_Z * configSetup.microsteps_Z) / 2);  // Amplitude is normally measured from the middle to the top
		...
		case ID_AXIS_X:
		{
			...
			if (configSetup.xAltX == 0)
			{
				slideStepsAmplitude = direction * (((configRose.amplitude_X / (configSetup.distancePerRev_X)) * configSetup.steps360_X * configSetup.microsteps_X) / 2);  // Amplitude is normally measured from the middle to the top
			}
			else
			{
				slideStepsAmplitude = direction * (((configRose.amplitude_X / (configSetup.distancePerRev_XAlt)) * configSetup.steps360_XAlt * configSetup.microsteps_XAlt) / 2);  // Amplitude is normally measured from the middle to the top
			}
		...
		case ID_AXIS_3:
		{
			...
			case RADIAL_M3:
			{
				slideStepsAmplitude = direction * (DistanceToSteps_M3Radial(configRose.amplitude_M3) / 2);
				break;
			}
			case LINEAR_M3:
			{
				slideStepsAmplitude = direction * (DistanceToSteps_M3Linear(configRose.amplitude_M3) / 2);
				break;
			}
			}
		...
		case ID_AXIS_4:
		{
			...
			case RADIAL_M4:
			{
				slideStepsAmplitude = direction * (DistanceToSteps_M4Radial(configRose.amplitude_M4) / 2);
				break;
			}
			case LINEAR_M4:
			{
				slideStepsAmplitude = direction * (DistanceToSteps_M4Linear(configRose.amplitude_M4) / 2);
				break;
			}
			}
...
void TestEEPROMSetup()  
{
	...
	const char* pageMore_c3_0 = "pageMore.c3.val=0";
	const char* pageMore_c3_1 = "pageMore.c3.val=1";
	const char* pageMore_c5_0 = "pageMore.c5.val=0";
	const char* pageMore_c5_1 = "pageMore.c5.val=1";
	const char* pageMore_c7_0 = "pageMore.c7.val=0";
	const char* pageMore_c7_1 = "pageMore.c7.val=1";
	const char* pageMore_c9_0 = "pageMore.c9.val=0";
	const char* pageMore_c9_1 = "pageMore.c9.val=1";
	const char* pageMore_c11_0 = "pageMore.c11.val=0";
	const char* pageMore_c11_1 = "pageMore.c11.val=1";
	...
	SerialWrite(0x22);
	SerialPrint(eePromPageSetup.microsteps_Z);
	SerialPrint(nextionQuoteEnd);

	SerialPrint(pageZ_t47);
	SerialWrite(0x22);
	SerialPrint(eePromPageSetup.steps360_Z);
	SerialPrint(nextionQuoteEnd);

	SerialPrint(pageZ_t48);
	SerialWrite(0x22);
	SerialPrint(eePromPageSetup.distancePerRev_Z, 6);
	SerialPrint(nextionQuoteEnd);
#ifdef DEBUG
	Serial.print("configSetup.distancePerRev_AxisZ:  ");
	Serial.println(configSetup.distancePerRev_Z,8);
	Serial.print("eePromPageSetup.distancePerRev_AxisZ:  ");
	Serial.println(eePromPageSetup.distancePerRev_Z,8);
#endif // DEBUG
	SerialPrint(pageZ_t49);
	SerialWrite(0x22);
	SerialPrint(eePromPageSetup.polarity_Z ? lowChar : highChar);
	SerialPrint(nextionQuoteEnd);

	// Page X
	SerialPrint(pageX_t60);
	SerialWrite(0x22);
	SerialPrint(eePromPageSetup.microsteps_X);
	SerialPrint(nextionQuoteEnd);

	SerialPrint(pageX_t61);
	SerialWrite(0x22);
	SerialPrint(eePromPageSetup.steps360_X);
	SerialPrint(nextionQuoteEnd);

	SerialPrint(pageX_t62);
	SerialWrite(0x22);
	SerialPrint(eePromPageSetup.distancePerRev_X, 5);
	SerialPrint(nextionQuoteEnd);

	SerialPrint(pageX_t63);
	SerialWrite(0x22);
	SerialPrint(eePromPageSetup.polarity_X ? lowChar : highChar);
	SerialPrint(nextionQuoteEnd);

	// Page AlternateX
	SerialPrint(pageAlternateX_t60);
	SerialWrite(0x22);
	SerialPrint(eePromPageSetup.microsteps_XAlt);
	SerialPrint(nextionQuoteEnd);

	SerialPrint(pageAlternateX_t61);
	SerialWrite(0x22);
	SerialPrint(eePromPageSetup.steps360_XAlt);
	SerialPrint(nextionQuoteEnd);

	SerialPrint(pageAlternateX_t62);
	SerialWrite(0x22);
	SerialPrint(eePromPageSetup.distancePerRev_XAlt, 5);
	SerialPrint(nextionQuoteEnd);

	SerialPrint(pageAlternateX_t63);
	SerialWrite(0x22);
	SerialPrint(eePromPageSetup.polarity_XAlt ? lowChar : highChar);
	SerialPrint(nextionQuoteEnd);
	...
	SerialWrite(0x22);
	SerialPrint(eePromPageSetup.microsteps_M3);
	SerialPrint(nextionQuoteEnd);

	SerialPrint(pageMotor_3_t57);
	SerialWrite(0x22);
	SerialPrint(eePromPageSetup.steps360_M3);
	SerialPrint(nextionQuoteEnd);

	SerialPrint(pageMotor_3_t58);
	SerialWrite(0x22);
	SerialPrint(eePromPageSetup.gearRatio_M3, 4);
	SerialPrint(nextionQuoteEnd);

	SerialPrint(pageMotor_3_t59);
	SerialWrite(0x22);
	SerialPrint(eePromPageSetup.polarity_M3 ? lowChar : highChar);
	SerialPrint(nextionQuoteEnd);

	// True=Linear, False = Radial
	if (eePromPageSetup.radialOrLinear_M3) 
	{
	...
	SerialWrite(0x22);
	SerialPrint(eePromPageSetup.distancePerRev_M3, 4);
	SerialPrint(nextionQuoteEnd);

	// pageMotor_4
	SerialPrint(pageMotor_4_t56);
	SerialWrite(0x22);
	SerialPrint(eePromPageSetup.microsteps_M4);
	SerialPrint(nextionQuoteEnd);

	SerialPrint(pageMotor_4_t57);
	SerialWrite(0x22);
	SerialPrint(configSetup.steps360_M4);
	SerialPrint(nextionQuoteEnd);

	SerialPrint(pageMotor_4_t58);
	SerialWrite(0x22);
	SerialPrint(eePromPageSetup.gearRatio_M4, 5);
	SerialPrint(nextionQuoteEnd);

	SerialPrint(pageMotor_4_t59);
	SerialWrite(0x22);
	SerialPrint(eePromPageSetup.polarity_M4 ? lowChar : highChar);
	SerialPrint(nextionQuoteEnd);

	// True=Linear, False = Radial
	if (eePromPageSetup.radialOrLinear_M4)
	{
	...
	SerialWrite(0x22);
	SerialPrint(eePromPageSetup.distancePerRev_M4, 4);
	SerialPrint(nextionQuoteEnd);
	...
	if (eePromPageSetup.keepSpindleEnabled == 0)
	{
		SerialPrint(pageMore_c3_0);
		SerialPrint(nextionEnd);

		SerialPrint("gkSpindleEnable=0");
		SerialPrint(nextionEnd);
	}
	else
	{
		SerialPrint(pageMore_c3_1);
		SerialPrint(nextionEnd);

		SerialPrint("gkSpindleEnable=1");
		SerialPrint(nextionEnd);
	}

	if (eePromPageSetup.keepZEnabled == 0)
	{
		SerialPrint(pageMore_c5_0);
		SerialPrint(nextionEnd);

		SerialPrint("gkZEnable=0");
		SerialPrint(nextionEnd);
	}
	else
	{
		SerialPrint(pageMore_c5_1);
		SerialPrint(nextionEnd);

		SerialPrint("gkZEnable=1");
		SerialPrint(nextionEnd);
	}

	if (eePromPageSetup.keepXEnabled == 0)
	{
		SerialPrint(pageMore_c7_0);
		SerialPrint(nextionEnd);

		SerialPrint("gkXEnable=0");
		SerialPrint(nextionEnd);
	}
	else
	{
		SerialPrint(pageMore_c7_1);
		SerialPrint(nextionEnd);

		SerialPrint("gkXEnable=1");
		SerialPrint(nextionEnd);
	}

	if (eePromPageSetup.keepM3Enabled == 0)
	{
		SerialPrint(pageMore_c9_0);
		SerialPrint(nextionEnd);

		SerialPrint("gkM3Enable=0");
		SerialPrint(nextionEnd);
	}
	else
	{
		SerialPrint(pageMore_c9_1);
		SerialPrint(nextionEnd);

		SerialPrint("gkM3Enable=1");
		SerialPrint(nextionEnd);
	}

	if (eePromPageSetup.keepM4Enabled == 0)
	{
		SerialPrint(pageMore_c11_0);
		SerialPrint(nextionEnd);

		SerialPrint("gkM4Enable=0");
		SerialPrint(nextionEnd);
	}
	else
	{
		SerialPrint(pageMore_c11_1);
		SerialPrint(nextionEnd);

		SerialPrint("gkM4Enable=1");
		SerialPrint(nextionEnd);
	}
...
void TestEEPROM_Returns() 
{
	...
	SerialWrite(0x22);
	SerialPrint(eePromPageSetup.maxSpd_Return_Z);
	SerialPrint(nextionQuoteEnd);

	SerialPrint(pageReturns_t3);
	SerialWrite(0x22);
	SerialPrint(eePromPageSetup.accel_Return_Z);
	SerialPrint(nextionQuoteEnd);

	SerialPrint(pageReturns_t7);
	SerialWrite(0x22);
	SerialPrint(eePromPageSetup.maxSpd_Return_X);
	SerialPrint(nextionQuoteEnd);

	SerialPrint(pageReturns_t8);
	SerialWrite(0x22);
	SerialPrint(eePromPageSetup.accel_Return_X);
	SerialPrint(nextionQuoteEnd);

	SerialPrint(pageReturns_t9);
	SerialWrite(0x22);
	SerialPrint(eePromPageSetup.maxSpd_Return_M3);
	SerialPrint(nextionQuoteEnd);

	SerialPrint(pageReturns_t10);
	SerialWrite(0x22);
	SerialPrint(eePromPageSetup.accel_Return_M3);
	SerialPrint(nextionQuoteEnd);

	SerialPrint(pageReturns_t15);
	SerialWrite(0x22);
	SerialPrint(eePromPageSetup.maxSpd_Return_M4);
	SerialPrint(nextionQuoteEnd);

	SerialPrint(pageReturns_t16);
	SerialWrite(0x22);
	SerialPrint(eePromPageSetup.accel_Return_M4);
	SerialPrint(nextionQuoteEnd);
...
void TestAllTeensyEEPROMValues() 
{
	...
	switch (pageCallerId)
	{
		case PAGE_MAIN:
		{
			...
			switch (configMain.axisId)
			{
				case ID_AXIS_Z:
				{
					...
					SerialPrint("pageEEPROM.t316.txt=");
					SerialWrite(0x22);
		...
		case PAGE_MULTI:
		{
			...
			SerialPrint("pageEEPROM.t316.txt=");
			SerialWrite(0x22);
			SerialPrint(configMulti.waves_Spindle);
			SerialPrint(nextionQuoteEnd);
			// Z Axis
			SerialPrint("pageEEPROM.t306.txt=");
			SerialWrite(0x22);
			...
			SerialPrint("pageEEPROM.t317.txt=");
			SerialWrite(0x22);
			SerialPrint(configMulti.waves_Z);
			SerialPrint(nextionQuoteEnd);
			// X Axis
			...
			SerialPrint(nextionQuoteEnd);
			SerialPrint("pageEEPROM.t517.txt=");
			SerialWrite(0x22);
			SerialPrint(configMulti.waves_X);
			SerialPrint(nextionQuoteEnd);
		...
		// M3 Axis
			...
			if (configSetup.radialOrLinear_M3 != 0)
			{
			...
			Serial.println(configSetup.radialOrLinear_M3);
			Serial.print("  dirAndChecked:  ");
			Serial.println(dirAndChecked);
#endif // DEBUG
			SerialPrint("pageEEPROM.t717.txt=");
			SerialWrite(0x22);
			SerialPrint(configMulti.waves_M3);
			SerialPrint(nextionQuoteEnd);
		// M4 Axis
			...
			Serial.println(configSetup.radialOrLinear_M4);
			Serial.print("  dirAndChecked:  ");
			Serial.println(dirAndChecked);
#endif // DEBUG
			...
			if (configSetup.radialOrLinear_M4 != 0)
			{
			...
			SerialPrint(nextionQuoteEnd);
			SerialPrint("pageEEPROM.t917.txt=");
			SerialWrite(0x22);
			SerialPrint(configMulti.waves_M4);
			SerialPrint(nextionQuoteEnd);
		...
		case PAGE_ROSE:
		{
			SerialPrint("pageEEPROM.t4.txt=");
			SerialWrite(0x22);
			SerialPrint("configRose.n");
			//rialPrint(configRose.n);
			SerialPrint(nextionQuoteEnd);
			...
			Serial.print("nnnnnnnnnnnnnnnnnnnnn: ");
			...
			switch (configRose.axisId)
			{
				case ID_AXIS_Z:
				{
					...
					SerialPrint("pageEEPROM.t309.txt=");
					SerialWrite(0x22);
					SerialPrint(configRose.amplitude_Z);
					SerialPrint(nextionQuoteEnd);
					break;
				}
				case ID_AXIS_X: // X Axis
				{
					...
					SerialPrint("pageEEPROM.t309.txt=");
					SerialWrite(0x22);
					SerialPrint(configRose.amplitude_X);
					SerialPrint(nextionQuoteEnd);
				...
				case ID_AXIS_3: // M3 Axis
				{
					...
					SerialPrint("pageEEPROM.t309.txt=");
					SerialWrite(0x22);
					SerialPrint(configRose.amplitude_M3);
					SerialPrint(nextionQuoteEnd);
					break;
				}
				case ID_AXIS_4: // M4 Axis
				{
					...
					SerialPrint("pageEEPROM.t309.txt=");
					SerialWrite(0x22);
					SerialPrint(configRose.amplitude_M4);
					SerialPrint(nextionQuoteEnd);
					break;
				}
			}
...
bool SprintF_Decimal(float num) 
{
	float modNum = 1.0;
	float checkNum = 0.0;
	return fmod(num, modNum) != checkNum;
}
...
void WriteSettingsToSD()
{
	...
	Serial.println("Enter WriteSettingsToSD");
	File fileSD = SD.open("Settings.ini", FILE_WRITE);
	fileSD.println("; Rose Engine Ini file");
	fileSD.println("; V3.1.2");
	...
	fileSD.println(currentValue);
	fileSD.println("; 0=Disabled, 1=Enabled");
	fileSD.print("KeepSteppersEnabled=");//0");
	sprintf(currentValue, "%lu", configSetup.keepSteppersEnabled);
	fileSD.println(currentValue);

	fileSD.print("KeepSpindleEnabled=");//0");
	sprintf(currentValue, "%lu", configSetup.keepSpindleEnabled);
	fileSD.println(currentValue);
	fileSD.print("KeepZEnabled=");//0");
	sprintf(currentValue, "%lu", configSetup.keepZEnabled);
	fileSD.println(currentValue);
	fileSD.print("KeepXEnabled=");//0");
	sprintf(currentValue, "%lu", configSetup.keepXEnabled);
	fileSD.println(currentValue);
	fileSD.print("KeepM3Enabled=");//0");
	sprintf(currentValue, "%lu", configSetup.keepM3Enabled);
	fileSD.println(currentValue);
	fileSD.print("KeepM4Enabled=");//0");
	sprintf(currentValue, "%lu", configSetup.keepM4Enabled);
	fileSD.println(currentValue);
	...
	if (SprintF_Decimal(configSetup.gearRatio_Spindle))
	{
		sprintf(currentValue, "%.5f", configSetup.gearRatio_Spindle);
	}
	else
	{
		noDecimal=(int32_t)configSetup.gearRatio_Spindle;
		sprintf(currentValue, "%lu", noDecimal);
	}
	fileSD.println(currentValue);
	...
	sprintf(currentValue, "%lu", configSetup.microsteps_Z);
	fileSD.println(currentValue);
	fileSD.print("StepsPer360_Z=");//200");
	sprintf(currentValue, "%lu", configSetup.steps360_Z);
	fileSD.println(currentValue);
	fileSD.print("DistancePer360_Z=");//0.187");
	//SprintF_Decimal(configSetup.distancePerRev_Z);
	//sprintf(currentValue, "%.5f", configSetup.distancePerRev_Z);
	if (SprintF_Decimal(configSetup.distancePerRev_Z))
	{
		sprintf(currentValue, "%.5f", configSetup.distancePerRev_Z);
	}
	else
	{
		noDecimal = (int32_t)configSetup.distancePerRev_Z;
		sprintf(currentValue, "%lu", noDecimal);
	}
	fileSD.println(currentValue);
	fileSD.print("Polarity_Z=");//1");
	sprintf(currentValue, "%d", configSetup.polarity_Z);
	fileSD.println(currentValue);
	...
	sprintf(currentValue, "%lu", configSetup.microsteps_X);
	fileSD.println(currentValue);
	fileSD.print("StepsPer360_X=");//200");
	sprintf(currentValue, "%lu", configSetup.steps360_X);
	fileSD.println(currentValue);
	fileSD.print("DistancePer360_X=");//0.187");
	//SprintF_Decimal( configSetup.distancePerRev_X);
	//sprintf(currentValue, "%.5f", configSetup.distancePerRev_X);
	if (SprintF_Decimal(configSetup.distancePerRev_X))
	{
		sprintf(currentValue, "%.5f", configSetup.distancePerRev_X);
	}
	else
	{
		noDecimal = (int32_t)configSetup.distancePerRev_X;
		sprintf(currentValue, "%lu", noDecimal);
	}
	fileSD.println(currentValue);
	fileSD.print("Polarity_X=");//1
	sprintf(currentValue, "%d", configSetup.polarity_X);
	fileSD.println(currentValue);
	...
	sprintf(currentValue, "%lu", configSetup.microsteps_XAlt);
	fileSD.println(currentValue);
	fileSD.print("StepsPer360_AltX=");//200
	sprintf(currentValue, "%lu", configSetup.steps360_XAlt);
	fileSD.println(currentValue);
	fileSD.print("DistancePer360_AltX=");//0.283
	//SprintF_Decimal(configSetup.distancePerRev_XAlt);
	//sprintf(currentValue, "%.5f", configSetup.distancePerRev_XAlt);
	 if (SprintF_Decimal(configSetup.distancePerRev_XAlt))
	{
		sprintf(currentValue, "%.5f", configSetup.distancePerRev_XAlt);
	}
	else
	{
		noDecimal = (int32_t)configSetup.distancePerRev_XAlt;
		sprintf(currentValue, "%lu", noDecimal);
	}
	fileSD.println(currentValue);
	fileSD.print("Polarity_AltX=");//1
	sprintf(currentValue, "%d", configSetup.polarity_XAlt);
	fileSD.println(currentValue);
	...
	sprintf(currentValue, "%lu", configSetup.microsteps_M3);
	fileSD.println(currentValue);
	fileSD.print("StepsPer360_M3=");//200
	sprintf(currentValue, "%lu", configSetup.steps360_M3);
	fileSD.println(currentValue);
	fileSD.print("GearRatio_M3=");//30
	//SprintF_Decimal(configSetup.gearRatio_M3);
	//sprintf(currentValue, "%.5f", configSetup.gearRatio_M3);
	 if (SprintF_Decimal(configSetup.gearRatio_M3))
	{
		sprintf(currentValue, "%.5f", configSetup.gearRatio_M3);
	}
	else
	{
		noDecimal = (int32_t)configSetup.gearRatio_M3;
		sprintf(currentValue, "%lu", noDecimal);
	}
	fileSD.println(currentValue);
	fileSD.print("Polarity_M3=");//1
	sprintf(currentValue, "%d", configSetup.polarity_M3);
	fileSD.println(currentValue);
	fileSD.print("DistancePer360_M3=");//1.02354
	//SprintF_Decimal(configSetup.distancePerRev_M3);
	//sprintf(currentValue, "%.5f", configSetup.distancePerRev_M3);
	if (SprintF_Decimal(configSetup.distancePerRev_M3))
	{
		sprintf(currentValue, "%.5f", configSetup.distancePerRev_M3);
	}
	else
	{
		noDecimal = (int32_t)configSetup.distancePerRev_M3;
		sprintf(currentValue, "%lu", noDecimal);
	}
	fileSD.println(currentValue);
	fileSD.print("RadialOrLineal_M3=");//0
	sprintf(currentValue, "%lu", configSetup.radialOrLinear_M3);
	fileSD.println(currentValue);
	fileSD.print("M3Radius=");//5.124
	//SprintF_Decimal(configSetup.radiusM3);
	//sprintf(currentValue, "%.5f", configSetup.radiusM3);
	if (SprintF_Decimal(configSetup.radiusM3))
	{
		sprintf(currentValue, "%.5f", configSetup.radiusM3);
	}
	else
	{
		noDecimal = (int32_t)configSetup.radiusM3;
		sprintf(currentValue, "%lu", noDecimal);
	}
	fileSD.println(currentValue);
	...
	sprintf(currentValue, "%lu", configSetup.microsteps_M4);
	fileSD.println(currentValue);
	fileSD.print("StepsPer360_M4=");//200
	sprintf(currentValue, "%lu", configSetup.steps360_M4);
	fileSD.println(currentValue);
	fileSD.print("GearRatio_M4=");//30
	//SprintF_Decimal(configSetup.gearRatio_M4);
	//sprintf(currentValue, "%.5f", configSetup.gearRatio_M4);
	if (SprintF_Decimal(configSetup.gearRatio_M4))
	{
		sprintf(currentValue, "%.5f", configSetup.gearRatio_M4);
	}
	else
	{
		noDecimal = (int32_t)configSetup.gearRatio_M4;
		sprintf(currentValue, "%lu", noDecimal);
	}
	fileSD.println(currentValue);
	fileSD.print("Polarity_M4=");//1
	sprintf(currentValue, "%d", configSetup.polarity_M4);
	fileSD.println(currentValue);
	fileSD.print("DistancePer360_M4=");//0.0254
	//SprintF_Decimal(configSetup.distancePerRev_M4);
	if (SprintF_Decimal(configSetup.distancePerRev_M4))
	{
		sprintf(currentValue, "%.5f", configSetup.distancePerRev_M4);
	}
	else
	{
		noDecimal = (int32_t)configSetup.distancePerRev_M4;
		sprintf(currentValue, "%lu", noDecimal);
	}
	fileSD.println(currentValue);
	fileSD.print("RadialOrLineal_M4=");//1
	sprintf(currentValue, "%lu", configSetup.radialOrLinear_M4);
	fileSD.println(currentValue);
	fileSD.print("M4Radius=");//11.258
	SprintF_Decimal(configSetup.radiusM4);
	//sprintf(currentValue, "%.5f", configSetup.radiusM4);
	if (SprintF_Decimal(configSetup.radiusM4))
	{
		sprintf(currentValue, "%.5f", configSetup.radiusM4);
	}
	else
	{
		noDecimal = (int32_t)configSetup.radiusM4;
		sprintf(currentValue, "%lu", noDecimal);
	}
	fileSD.println(currentValue);
	fileSD.println(";");
...
////	char currentValue[60];
	...
	sprintf(currentValue, "%lu", configSetup.maxSpd_Return_Z);
	fileSD.println(currentValue);
	fileSD.print("Accel_Axis_Z="); //15000
	sprintf(currentValue, "%lu", configSetup.accel_Return_Z);
	fileSD.println(currentValue);
	fileSD.print("MaxSpeed_Axis_X="); //30000
	sprintf(currentValue, "%lu", configSetup.maxSpd_Return_X);
	fileSD.println(currentValue);
	fileSD.print("Accel_Axis_X="); //15000
	sprintf(currentValue, "%lu", configSetup.accel_Return_X);
	fileSD.println(currentValue);
	fileSD.print("MaxSpeed_Axis_M3="); //30000
	sprintf(currentValue, "%lu", configSetup.maxSpd_Return_M3);
	fileSD.println(currentValue);
	fileSD.print("Accel_Axis_M3="); //15000
	sprintf(currentValue, "%lu", configSetup.accel_Return_M3);
	fileSD.println(currentValue);
	fileSD.print("MaxSpeed_Axis_M4="); //30000
	sprintf(currentValue, "%lu", configSetup.maxSpd_Return_M4);
	fileSD.println(currentValue);
	fileSD.print("Accel_Axis_M4="); //15000
	sprintf(currentValue, "%lu", configSetup.accel_Return_M4);
	fileSD.println(currentValue);
...
////void WriteSettings_Multi()
	...
	fileSD.println("[MultiSync]");
	fileSD.println(";");
	fileSD.println(";Settings are shared between One and MultiSync pages.");
	fileSD.print("AxisId="); //1
	sprintf(currentValue, "%ld", configMulti.axisId);
	fileSD.println(currentValue);
	//fileSD.println(";Checked axes ANDed together and stored in one variable.");
	//fileSD.print("Checked_AxesIDs="); //0
	...
	if (SprintF_Decimal(configMulti.targetDistance_Spindle))
	{
		sprintf(currentValue, "%.5f", configMulti.targetDistance_Spindle);
	}
	else
	{
		noDecimal = (int32_t)configMulti.targetDistance_Spindle;
		sprintf(currentValue, "%lu", noDecimal);
	}
	fileSD.println(currentValue);
	...
	fileSD.print("Waves_Spindle="); //5068
	sprintf(currentValue, "%lu", configMulti.waves_Spindle);
	fileSD.println(currentValue);
	// Checked
	fileSD.print("Checked_Spindle=");
	sprintf(currentValue, "%lu", configMulti.checked_Spindle);
	fileSD.println(currentValue);
	// Direction
	fileSD.println("; CW = 2, CCW = 3");
	if (configMulti.direction_Spindle == 1)
	{
		fileSD.println("Direction_Spindle=2");
	}
	else
	{
		fileSD.println("Direction_Spindle=3");
	}
	...
	//sprintf(currentValue, "%.5f", configMulti.targetDistance_Z);
	if (SprintF_Decimal(configMulti.targetDistance_Z))
	{
		sprintf(currentValue, "%.5f", configMulti.targetDistance_Z);
	}
	else
	{
		noDecimal = (int32_t)configMulti.targetDistance_Z;
		sprintf(currentValue, "%lu", noDecimal);
	}
	fileSD.println(currentValue);

	fileSD.print("Waves_Z="); //5068
	sprintf(currentValue, "%lu", configMulti.waves_Z);
	fileSD.println(currentValue);
	// Checked
	fileSD.print("Checked_Z=");
	sprintf(currentValue, "%lu", configMulti.checked_Z);
	fileSD.println(currentValue);
	// Direction
	fileSD.println("; Right=2, Left=3");

	if (configMulti.direction_Z == 1)
	{
		fileSD.println("Direction_Z=2");
	}
	else
	{
		fileSD.println("Direction_Z=3");
	}
	...
	//sprintf(currentValue, "%.5f", configMulti.targetDistance_X);
	if (SprintF_Decimal(configMulti.targetDistance_X))
	{
		sprintf(currentValue, "%.5f", configMulti.targetDistance_X);
	}
	else
	{
		noDecimal = (int32_t)configMulti.targetDistance_X;
		sprintf(currentValue, "%lu", noDecimal);
	}
	fileSD.println(currentValue);

	fileSD.print("Waves_X="); //5068
	sprintf(currentValue, "%lu", configMulti.waves_X);
	fileSD.println(currentValue);
	// Checked
	fileSD.print("Checked_X=");
	sprintf(currentValue, "%lu", configMulti.checked_X);
	fileSD.println(currentValue);
	// Direction
	fileSD.println("; Right=2, Left=3");
	...
	if (configMulti.direction_X == 1)
	{
		fileSD.println("Direction_X=2");
	}
	else
	{
		fileSD.println("Direction_X=3");
	}
	...
	//sprintf(currentValue, "%.5f", configMulti.targetDistance_M3);
	if (SprintF_Decimal(configMulti.targetDistance_M3))
	{
		sprintf(currentValue, "%.5f", configMulti.targetDistance_M3);
	}
	else
	{
		noDecimal = (int32_t)configMulti.targetDistance_M3;
		sprintf(currentValue, "%lu", noDecimal);
	}
	fileSD.println(currentValue);

	fileSD.print("Waves_M3="); //5068
	sprintf(currentValue, "%lu", configMulti.waves_M3);
	fileSD.println(currentValue);
	// Checked
	fileSD.print("Checked_M3=");
	sprintf(currentValue, "%lu", configMulti.checked_M3);
	fileSD.println(currentValue);
	// Direction
	fileSD.println("; Right and CW = 2, Left and CCW = 3");

	if (configMulti.direction_M3 == 1)
	{
		fileSD.println("Direction_M3=2");
	}
	else
	{
		fileSD.println("Direction_M3=3");
	}
	...
	//sprintf(currentValue, "%.5f", configMulti.targetDistance_M4);
	if (SprintF_Decimal(configMulti.targetDistance_M4))
	{
		sprintf(currentValue, "%.5f", configMulti.targetDistance_M4);
	}
	else
	{
		noDecimal = (int32_t)configMulti.targetDistance_M4;
		sprintf(currentValue, "%lu", noDecimal);
	}
	fileSD.println(currentValue);

	fileSD.print("Waves_M4="); //5068
	sprintf(currentValue, "%lu", configMulti.waves_M4);
	fileSD.println(currentValue);
	// Checked
	fileSD.print("Checked_M4=");
	sprintf(currentValue, "%lu", configMulti.checked_M4);
	fileSD.println(currentValue);
	// Direction
	fileSD.println("; Right and CW = 2, Left and CCW = 3");
	...
	if (configMulti.direction_M4 == 1)
	{
		fileSD.println("Direction_M4=2");
	}
	else
	{
		fileSD.println("Direction_M4=3");
	}
...
////void WriteSettings_Index()
	...
	fileSD.println("; Axis Ids : Spindle = 0,  M3 = 3, M4 = 4");
	fileSD.print("AxisId="); //1
	...
	//sprintf(currentValue, "%.5f", configIndex_Prime.synchro_M3_Percentage);
	if (SprintF_Decimal(configIndex_Prime.synchro_M3_Percentage))
	{
		sprintf(currentValue, "%.5f", configIndex_Prime.synchro_M3_Percentage);
	}
	else
	{
		noDecimal = (int32_t)configIndex_Prime.synchro_M3_Percentage;
		sprintf(currentValue, "%lu", noDecimal);
	}
	fileSD.println(currentValue);
	fileSD.println("; Spindle");
	...
	//sprintf(currentValue, "%.5f", configIndex_1.sizeInUnits);
	if (SprintF_Decimal(configIndex_1.sizeInUnits))
	{
		sprintf(currentValue, "%.5f", configIndex_1.sizeInUnits);
	}
	else
	{
		noDecimal = (int32_t)configIndex_1.sizeInUnits;
		sprintf(currentValue, "%lu", noDecimal);
	}
	fileSD.println(currentValue);
	...
	//sprintf(currentValue, "%.5f", configIndex_2.sizeInUnits);
	if (SprintF_Decimal(configIndex_2.sizeInUnits))
	{
		sprintf(currentValue, "%.5f", configIndex_2.sizeInUnits);
	}
	else
	{
		noDecimal = (int32_t)configIndex_2.sizeInUnits;
		sprintf(currentValue, "%lu", noDecimal);
	}
	fileSD.println(currentValue);
	...
	//sprintf(currentValue, "%.5f", configIndex_3.sizeInUnits);
	if (SprintF_Decimal(configIndex_3.sizeInUnits))
	{
		sprintf(currentValue, "%.5f", configIndex_3.sizeInUnits);
	}
	else
	{
		noDecimal = (int32_t)configIndex_3.sizeInUnits;
		sprintf(currentValue, "%lu", noDecimal);
	}
	fileSD.println(currentValue);
	...
	//sprintf(currentValue, "%.5f", configIndex_4.sizeInUnits);
	if (SprintF_Decimal(configIndex_4.sizeInUnits))
	{
		sprintf(currentValue, "%.5f", configIndex_4.sizeInUnits);
	}
	else
	{
		noDecimal = (int32_t)configIndex_4.sizeInUnits;
		sprintf(currentValue, "%lu", noDecimal);
	}
	fileSD.println(currentValue);
	...
	//sprintf(currentValue, "%.5f", configIndex_5.sizeInUnits);
	if (SprintF_Decimal(configIndex_5.sizeInUnits))
	{
		sprintf(currentValue, "%.5f", configIndex_5.sizeInUnits);
	}
	else
	{
		noDecimal = (int32_t)configIndex_5.sizeInUnits;
		sprintf(currentValue, "%lu", noDecimal);
	}
	fileSD.println(currentValue);
	fileSD.println(";");
...
////void WriteSettings_Move()
	...
	//sprintf(currentValue, "%.5f", configMove.distance_MoveZ1);
	if (SprintF_Decimal(configMove.distance_MoveZ1))
	{
		sprintf(currentValue, "%.5f", configMove.distance_MoveZ1);
	}
	else
	{
		noDecimal = (int32_t)configMove.distance_MoveZ1;
		sprintf(currentValue, "%lu", noDecimal);
	}
	fileSD.println(currentValue);
	...
	//sprintf(currentValue, "%.5f", configMove.distance_MoveZ2);
	if (SprintF_Decimal(configMove.distance_MoveZ2))
	{
		sprintf(currentValue, "%.5f", configMove.distance_MoveZ2);
	}
	else
	{
		noDecimal = (int32_t)configMove.distance_MoveZ2;
		sprintf(currentValue, "%lu", noDecimal);
	}
	fileSD.println(currentValue);
	fileSD.println("; X Axis");
	...
	//sprintf(currentValue, "%.5f", configMove.distance_MoveX1);
	if (SprintF_Decimal(configMove.distance_MoveX1))
	{
		sprintf(currentValue, "%.5f", configMove.distance_MoveX1);
	}
	else
	{
		noDecimal = (int32_t)configMove.distance_MoveX1;
		sprintf(currentValue, "%lu", noDecimal);
	}
	fileSD.println(currentValue);
	fileSD.print("Distance_X2="); //5.76
	//sprintf(currentValue, "%.5f", configMove.distance_MoveX2);
	if (SprintF_Decimal(configMove.distance_MoveX2))
	{
		sprintf(currentValue, "%.5f", configMove.distance_MoveX2);
	}
	else
	{
		noDecimal = (int32_t)configMove.distance_MoveX2;
		sprintf(currentValue, "%lu", noDecimal);
	}
	fileSD.println(currentValue);
	fileSD.println("; M3 Axis");
	...
	//sprintf(currentValue, "%.5f", configMove.distance_MoveM3_1);
	if (SprintF_Decimal(configMove.distance_MoveM3_1))
	{
		sprintf(currentValue, "%.5f", configMove.distance_MoveM3_1);
	}
	else
	{
		noDecimal = (int32_t)configMove.distance_MoveM3_1;
		sprintf(currentValue, "%lu", noDecimal);
	}
	fileSD.println(currentValue);
	fileSD.print("Distance_M3 - 2="); //5.79
	//sprintf(currentValue, "%.5f", configMove.distance_MoveM3_2);
	if (SprintF_Decimal(configMove.distance_MoveM3_2))
	{
		sprintf(currentValue, "%.5f", configMove.distance_MoveM3_2);
	}
	else
	{
		noDecimal = (int32_t)configMove.distance_MoveM3_2;
		sprintf(currentValue, "%lu", noDecimal);
	}
	fileSD.println(currentValue);
	fileSD.println("; M4 Axis");
	...
	//sprintf(currentValue, "%.5f", configMove.distance_MoveM4_1);
	if (SprintF_Decimal(configMove.distance_MoveM4_1))
	{
		sprintf(currentValue, "%.5f", configMove.distance_MoveM4_1);
	}
	else
	{
		noDecimal = (int32_t)configMove.distance_MoveM4_1;
		sprintf(currentValue, "%lu", noDecimal);
	}
	fileSD.println(currentValue);
	fileSD.print("Distance_M4 - 2="); //4.79
	//sprintf(currentValue, "%.5f", configMove.distance_MoveM4_2);
	if (SprintF_Decimal(configMove.distance_MoveM4_2))
	{
		sprintf(currentValue, "%.5f", configMove.distance_MoveM4_2);
	}
	else
	{
		noDecimal = (int32_t)configMove.distance_MoveM4_2;
		sprintf(currentValue, "%lu", noDecimal);
	}
	fileSD.println(currentValue);
	...
	// Shared settings
	fileSD.println("[Shared]");
	fileSD.println(";");

	fileSD.println("; Settings shared between");
	fileSD.println("; Main, Sync, Recip, Rose, Greek Key, and Program pages");
	fileSD.println("; Synchronize M3 and Spindle");
	fileSD.print("Synchro_M3_and_Spindle=");
	sprintf(currentValue, "%lu", configMain.synchro_M3_Spindle);
	fileSD.println(currentValue);
	fileSD.print("Synchro_Percentage=");
	//sprintf(currentValue, "%.5f", configMain.synchro_M3_Percentage);
	if (SprintF_Decimal(configMain.synchro_M3_Percentage))
	{
		sprintf(currentValue, "%.5f", configMain.synchro_M3_Percentage);
	}
	else
	{
		noDecimal = (int32_t)configMain.synchro_M3_Percentage;
		sprintf(currentValue, "%lu", noDecimal);
	}
	fileSD.println(currentValue);
	...
	fileSD.println("; Settings shared between");
	fileSD.println("; Sync, Recip, Greek Key, and Program pages");
	...
	fileSD.print("Revolutions_or_Degrees="); //0
	sprintf(currentValue, "%lu", configSync.spindle_RevolutionsOrDegrees);
	fileSD.println(currentValue);

	fileSD.print("Revolutions="); //0.3	
	//sprintf(currentValue, "%.5f", configSync.revolutions_Spindle);
	if (SprintF_Decimal(configSync.revolutions_Spindle))
	{
		sprintf(currentValue, "%.5f", configSync.revolutions_Spindle);
	}
	else
	{
		noDecimal = (int32_t)configSync.revolutions_Spindle;
		sprintf(currentValue, "%lu", noDecimal);
	}
	fileSD.println(currentValue);
	...
	//sprintf(currentValue, "%.5f", configSync.distance);
	if (SprintF_Decimal(configSync.distance))
	{
		sprintf(currentValue, "%.5f", configSync.distance);
	}
	else
	{
		noDecimal = (int32_t)configSync.distance;
		sprintf(currentValue, "%lu", noDecimal);
	}
	fileSD.println(currentValue);
...
////void WriteSettings_Recip()
	...
	//sprintf(currentValue, "%.5f", configRec.degrees_Radial_Spindle);
	if (SprintF_Decimal(configRec.degrees_Radial_Spindle))
	{
		sprintf(currentValue, "%.5f", configRec.degrees_Radial_Spindle);
	}
	else
	{
		noDecimal = (int32_t)configRec.degrees_Radial_Spindle;
		sprintf(currentValue, "%lu", noDecimal);
	}
	fileSD.println(currentValue);
	...
	//sprintf(currentValue, "%.5f", configRec.amplitude_Radial_Axis);
	if (SprintF_Decimal(configRec.amplitude_Radial_Axis))
	{
		sprintf(currentValue, "%.5f", configRec.amplitude_Radial_Axis);
	}
	else
	{
		noDecimal = (int32_t)configRec.amplitude_Radial_Axis;
		sprintf(currentValue, "%lu", noDecimal);
	}
	fileSD.println(currentValue);
	...
	//sprintf(currentValue, "%.5f", configRec.amplitude_Axial_Spindle);
	if (SprintF_Decimal(configRec.amplitude_Axial_Spindle))
	{
		sprintf(currentValue, "%.5f", configRec.amplitude_Axial_Spindle);
	}
	else
	{
		noDecimal = (int32_t)configRec.amplitude_Axial_Spindle;
		sprintf(currentValue, "%lu", noDecimal);
	}
	fileSD.println(currentValue);
	fileSD.print("Axial_Axis_Distance="); //7.896
	//sprintf(currentValue, "%.5f", configRec.distance_Axial_Axis);
	if (SprintF_Decimal(configRec.distance_Axial_Axis))
	{
		sprintf(currentValue, "%.5f", configRec.distance_Axial_Axis);
	}
	else
	{
		noDecimal = (int32_t)configRec.distance_Axial_Axis;
		sprintf(currentValue, "%lu", noDecimal);
	}
	fileSD.println(currentValue);

	fileSD.print("RetroSpindleDistance="); //0
	//sprintf(currentValue, "%.5f", configRec.retro_SpindleChange);
	if (SprintF_Decimal(configRec.retro_SpindleChange))
	{
		sprintf(currentValue, "%.5f", configRec.retro_SpindleChange);
	}
	else
	{
		noDecimal = (int32_t)configRec.retro_SpindleChange;
		sprintf(currentValue, "%lu", noDecimal);
	}
	fileSD.println(currentValue);

	fileSD.print("RetroAxisDistance="); //0
	//sprintf(currentValue, "%.5f", configRec.retro_AxisChange);
	if (SprintF_Decimal(configRec.retro_AxisChange))
	{
		sprintf(currentValue, "%.5f", configRec.retro_AxisChange);
	}
	else
	{
		noDecimal = (int32_t)configRec.retro_AxisChange;
		sprintf(currentValue, "%lu", noDecimal);
	}
	fileSD.println(currentValue);
...
////void WriteSettings_GreekKey()
	...
	//sprintf(currentValue, "%.5f", configGreekKey.countPatternPer360_Pattern);
	if (SprintF_Decimal(configGreekKey.countPatternPer360_Pattern))
	{
		sprintf(currentValue, "%.5f", configGreekKey.countPatternPer360_Pattern);
	}
	else
	{
		noDecimal = (int32_t)configGreekKey.countPatternPer360_Pattern;
		sprintf(currentValue, "%lu", noDecimal);
	}
	fileSD.println(currentValue);
	fileSD.print("PatternCount="); //24
	sprintf(currentValue, "%lu", configGreekKey.patternCount_Pattern);
	fileSD.println(currentValue);
	fileSD.print("SegmentLength="); //1.2
	//sprintf(currentValue, "%.5f", configGreekKey.segmentLength_Pattern);
	if (SprintF_Decimal(configGreekKey.segmentLength_Pattern))
	{
		sprintf(currentValue, "%.5f", configGreekKey.segmentLength_Pattern);
	}
	else
	{
		noDecimal = (int32_t)configGreekKey.segmentLength_Pattern;
		sprintf(currentValue, "%lu", noDecimal);
	}
	fileSD.println(currentValue);
...
////void WriteSettings_Program()
	...
	//sprintf(currentValue, "%.5f", configGreekKey.countPatternPer360_File);
	if (SprintF_Decimal(configGreekKey.countPatternPer360_File))
	{
		sprintf(currentValue, "%.5f", configGreekKey.countPatternPer360_File);
	}
	else
	{
		noDecimal = (int32_t)configGreekKey.countPatternPer360_File;
		sprintf(currentValue, "%lu", noDecimal);
	}
	fileSD.println(currentValue);
	fileSD.print("PatternCount="); //12
	sprintf(currentValue, "%lu", configGreekKey.patternCount_File);
	fileSD.println(currentValue);
	fileSD.print("SegmentLength="); //0.032
	sprintf(currentValue, "%.5f", configGreekKey.segmentLength_File);
	fileSD.println(currentValue);
	fileSD.println("; Segments: 2  Actual : 3");
	fileSD.print("SegmentsOrActual="); // 2
	//sprintf(currentValue, "%lu", configGreekKey.segmentOrActual);
	if (SprintF_Decimal(configGreekKey.segmentOrActual))
	{
		sprintf(currentValue, "%.5f", (double)configGreekKey.segmentOrActual);
	}
	else
	{
		noDecimal = (int32_t)configGreekKey.segmentOrActual;
		sprintf(currentValue, "%lu", noDecimal);
	}
	fileSD.println(currentValue);
...
////void WriteSettings_Rose()
	...
	//sprintf(currentValue, "%.5f", configRose.n);
	if (SprintF_Decimal(configRose.n))
	{
		sprintf(currentValue, "%.5f", configRose.n);
	}
	else
	{
		noDecimal = (int32_t)configRose.n;
		sprintf(currentValue, "%lu", noDecimal);
	}
	fileSD.println(currentValue);
	fileSD.print("Rose_d="); //5
	//sprintf(currentValue, "%.5f", configRose.d);
	if (SprintF_Decimal(configRose.d))
	{
		sprintf(currentValue, "%.5f", configRose.d);
	}
	else
	{
		noDecimal = (int32_t)configRose.d;
		sprintf(currentValue, "%lu", noDecimal);
	}
	fileSD.println(currentValue);
	fileSD.println("; Spindle");
	fileSD.print("SpindleRevolutions="); //3.125
	//sprintf(currentValue, "%.5f", configRose.spindleRevolutions);
	if (SprintF_Decimal(configRose.spindleRevolutions))
	{
		sprintf(currentValue, "%.5f", configRose.spindleRevolutions);
	}
	else
	{
		noDecimal = (int32_t)configRose.spindleRevolutions;
		sprintf(currentValue, "%lu", noDecimal);
	}
	fileSD.println(currentValue);
	...
	fileSD.println(currentValue);
	fileSD.print("Amplitude_Z="); //2.687
	//sprintf(currentValue, "%.5f", configRose.amplitude_Radial_Z);
	if (SprintF_Decimal(configRose.amplitude_Z))
	{
		sprintf(currentValue, "%.5f", configRose.amplitude_Z);
	}
	else
	{
		noDecimal = (int32_t)configRose.amplitude_Z;
		sprintf(currentValue, "%lu", noDecimal);
	}
	fileSD.println(currentValue);
	fileSD.println("; X Axis");
	...
	fileSD.println(currentValue);
	fileSD.print("Amplitude_X="); //2.5
	//sprintf(currentValue, "%.5f", configRose.amplitude_Radial_X);
	if (SprintF_Decimal(configRose.amplitude_X))
	{
		sprintf(currentValue, "%.5f", configRose.amplitude_X);
	}
	else
	{
		noDecimal = (int32_t)configRose.amplitude_X;
		sprintf(currentValue, "%lu", noDecimal);
	}
	fileSD.println(currentValue);
	fileSD.println("; M3 Axis");
	...
	fileSD.println(currentValue);
	fileSD.print("Amplitude_M3="); //3.5
	//sprintf(currentValue, "%.5f", configRose.amplitude_Radial_M3);
	if (SprintF_Decimal(configRose.amplitude_M3))
	{
		sprintf(currentValue, "%.5f", configRose.amplitude_M3);
	}
	else
	{
		noDecimal = (int32_t)configRose.amplitude_M3;
		sprintf(currentValue, "%lu", noDecimal);
	}
	fileSD.println(currentValue);
	fileSD.println("; M4 Axis");
	...
	fileSD.println(currentValue);
	fileSD.print("Amplitude_M4="); //3.568
	//sprintf(currentValue, "%.5f", configRose.amplitude_Radial_M4);
	if (SprintF_Decimal(configRose.amplitude_M4))
	{
		sprintf(currentValue, "%.5f", configRose.amplitude_M4);
	}
	else
	{
		noDecimal = (int32_t)configRose.amplitude_M4;
		sprintf(currentValue, "%lu", noDecimal);
	}
	fileSD.println(currentValue);
...
fileSD.close();
Serial.println("Settings.ini Complete.");

SerialPrint("vis t3,1");
SerialPrint(nextionEnd);
...
void LoadSettings_PageIndex() 
{
	...
	returnVal = GetIniValue(iniKey, iniValue, eePromAddress_Nextion, false);
	configIndex_Prime.accel_M3 = (int)returnVal;

	iniValue = "MaxSpeed_M3";
	eePromAddress_Nextion = 804;
	returnVal = GetIniValue(iniKey, iniValue, eePromAddress_Nextion, false);
	configIndex_Prime.maxSpd_M3 = (int)returnVal;

	iniValue = "Accel_M4";
	eePromAddress_Nextion = 984;
	returnVal = GetIniValue(iniKey, iniValue, eePromAddress_Nextion, false);
	configIndex_Prime.accel_M4 = (int)returnVal;

	iniValue = "MaxSpeed_M4";
	eePromAddress_Nextion = 792;
	returnVal = GetIniValue(iniKey, iniValue, eePromAddress_Nextion, false);
	configIndex_Prime.maxSpd_M4 = (int)returnVal;
...
// EEPROM 100
	...
	// Axis Speed %
    // EEPROM 52
	value0 = (uint8_t)configRose.speedPercent_Z;
	value8 = (uint8_t)configRose.speedPercent_X;
	value16 = (uint8_t)configRose.speedPercent_M3;
	value24 = (uint8_t)configRose.speedPercent_M4;

	eePromAddress_Nextion = 52;
	iniValue = "SpeedPercentage_Z";
	returnVal = ReturnIniValue(iniKey, iniValue);
	configRose.speedPercent_Z = (int)returnVal;
	value0 = (uint8_t)configRose.speedPercent_Z;

	iniValue = "SpeedPercentage_X";
	returnVal = ReturnIniValue(iniKey, iniValue);
	configRose.speedPercent_X = (int)returnVal;
	value8 = (uint8_t)configRose.speedPercent_X;

	iniValue = "SpeedPercentage_M3";
	returnVal = ReturnIniValue(iniKey, iniValue);
	configRose.speedPercent_M3 = (int)returnVal;
	value16 = (uint8_t)configRose.speedPercent_M3;

	iniValue = "SpeedPercentage_M4";
	returnVal = ReturnIniValue(iniKey, iniValue);
	configRose.speedPercent_M4 = (int)returnVal;
	value24 = (uint8_t)configRose.speedPercent_M4;
	...
	returnVal = GetIniValue(iniKey, iniValue, eePromAddress_Nextion, false);
	configRose.maxSpd_Z = (int)returnVal;

	iniValue = "Accel_Z";
	eePromAddress_Nextion = 948;
	returnVal = GetIniValue(iniKey, iniValue, eePromAddress_Nextion, false);
	configRose.accel_Z = (int)returnVal;

	iniValue = "Amplitude_Z";
	eePromAddress_Nextion = 276;
	returnVal = GetIniValue(iniKey, iniValue, eePromAddress_Nextion, true);
	configRose.amplitude_Z = returnVal;
	MilliDelay(3000);
	...
	returnVal = GetIniValue(iniKey, iniValue, eePromAddress_Nextion, false);
	configRose.maxSpd_X = (int)returnVal;

	iniValue = "Accel_X";
	eePromAddress_Nextion = 868;
	returnVal = GetIniValue(iniKey, iniValue, eePromAddress_Nextion, false);
	configRose.accel_X = (int)returnVal;

	iniValue = "Amplitude_X";
	eePromAddress_Nextion = 284;
	returnVal = GetIniValue(iniKey, iniValue, eePromAddress_Nextion, true);
	configRose.amplitude_X = returnVal;
	...
	returnVal = GetIniValue(iniKey, iniValue, eePromAddress_Nextion, false);
	configRose.maxSpd_M3 = (int)returnVal;

	iniValue = "Accel_M3";
	eePromAddress_Nextion = 820;
	returnVal = GetIniValue(iniKey, iniValue, eePromAddress_Nextion, false);
	configRose.accel_M3 = (int)returnVal;

	iniValue = "Amplitude_M3";
	eePromAddress_Nextion = 520;
	returnVal = GetIniValue(iniKey, iniValue, eePromAddress_Nextion, true);
	configRose.amplitude_M3 = returnVal;
	...
	returnVal = GetIniValue(iniKey, iniValue, eePromAddress_Nextion, false);
	configRose.maxSpd_M4 = (int)returnVal;

	iniValue = "Accel_M4";
	eePromAddress_Nextion = 844;
	returnVal = GetIniValue(iniKey, iniValue, eePromAddress_Nextion, false);
	configRose.accel_M4 = (int)returnVal;

	iniValue = "Amplitude_M4";
	eePromAddress_Nextion = 68;
	returnVal = GetIniValue(iniKey, iniValue, eePromAddress_Nextion, true);
	configRose.amplitude_M4 = returnVal;
...
void LoadSettings_PageMove()
{
	...
	// Speed %
	// EEPROM 348
	value0 = (uint8_t)configMove.speedPercent_Z;
	value8 = (uint8_t)configMove.speedPercent_X;
	value16 = (uint8_t)configMove.speedPercent_M3;
	value24 = (uint8_t)configMove.speedPercent_M4;
	...
	value0 = configMove.speedPercent_Z;
...
void LoadSettings_PageMulti()
{
	...
	returnVal = ReturnIniValue(iniKey, iniValue);
	configMulti.speedPercent_Z = (int)returnVal;
	value0 = (uint8_t)configMulti.speedPercent_Z;

	iniValue = "SpeedPercentage_X";
	returnVal = ReturnIniValue(iniKey, iniValue);
	configMulti.speedPercent_X = (int)returnVal;
	value8 = (uint8_t)configMulti.speedPercent_X;

	iniValue = "SpeedPercentage_M3";
	returnVal = ReturnIniValue(iniKey, iniValue);
	configMulti.speedPercent_M3 = (int)returnVal;
	value16 = (uint8_t)configMulti.speedPercent_M3;

	iniValue = "SpeedPercentage_M4";
	returnVal = ReturnIniValue(iniKey, iniValue);
	configMulti.speedPercent_M4 = (int)returnVal;
	value24 = (uint8_t)configMulti.speedPercent_M4;
	...
	Serial.print("----------------------------------configMulti.waves_Spindle");
	Serial.println(configMulti.waves_Spindle);
	iniValue = "Waves_Spindle";
	eePromAddress_Nextion = 148;
	returnVal = GetIniValue(iniKey, iniValue, eePromAddress_Nextion, false);
	configMulti.waves_Spindle = returnVal;

	iniValue = "Waves_Z";
	eePromAddress_Nextion = 44;
	returnVal = GetIniValue(iniKey, iniValue, eePromAddress_Nextion, false);
	configMulti.waves_Z = returnVal;

	iniValue = "Waves_X";
	eePromAddress_Nextion = 56;
	returnVal = GetIniValue(iniKey, iniValue, eePromAddress_Nextion, false);
	configMulti.waves_X = returnVal;

	iniValue = "Waves_M3";
	eePromAddress_Nextion = 84;
	returnVal = GetIniValue(iniKey, iniValue, eePromAddress_Nextion, false);
	configMulti.waves_M3 = returnVal;

	iniValue = "Waves_M4";
	eePromAddress_Nextion = 108;
	returnVal = GetIniValue(iniKey, iniValue, eePromAddress_Nextion, false);
	configMulti.waves_M4 = returnVal;

	// Checked
	iniValue = "Checked_Spindle";
	eePromAddress_Nextion = 420;
	returnVal = GetIniValue(iniKey, iniValue, eePromAddress_Nextion, false);
	configMulti.checked_Spindle = returnVal;

	iniValue = "Checked_Z";
	eePromAddress_Nextion = 208;
	returnVal = GetIniValue(iniKey, iniValue, eePromAddress_Nextion, false);
	configMulti.checked_Z = returnVal;

	iniValue = "Checked_X";
	eePromAddress_Nextion = 164;
	returnVal = GetIniValue(iniKey, iniValue, eePromAddress_Nextion, false);
	configMulti.checked_X = returnVal;

	iniValue = "Checked_M3";
	eePromAddress_Nextion = 212;
	returnVal = GetIniValue(iniKey, iniValue, eePromAddress_Nextion, false);
	configMulti.checked_M3 = returnVal;

	iniValue = "Checked_M4";
	eePromAddress_Nextion = 376;
	returnVal = GetIniValue(iniKey, iniValue, eePromAddress_Nextion, false);
	configMulti.checked_M4 = returnVal;
...
void LoadSettings_PageMain()
{
	...
	value0 = configMain.speedPercent_Z;
	value8 = configMain.speedPercent_X;
	value16 = configMain.speedPercent_M3;
	value24 = configMain.speedPercent_M4;
	eePromAddress_Nextion = 8;

	iniValue = "SpeedPercentage_Z"; // 0
	returnVal = ReturnIniValue(iniKey, iniValue);
	configMain.speedPercent_Z = (int)returnVal;
	value0 = (uint8_t)configMain.speedPercent_Z;

	iniValue = "SpeedPercentage_X"; // 8
	returnVal = ReturnIniValue(iniKey, iniValue);
	configMain.speedPercent_X = (int)returnVal;
	value8 = (uint8_t)configMain.speedPercent_X;

	iniValue = "SpeedPercentage_M3"; // 16
	returnVal = ReturnIniValue(iniKey, iniValue);
	configMain.speedPercent_M3 = (int)returnVal;
	value16 = (uint8_t)configMain.speedPercent_M3;

	iniValue = "SpeedPercentage_M4"; // 24
	returnVal = ReturnIniValue(iniKey, iniValue);
	configMain.speedPercent_M4 = (int)returnVal;
	value24 = (uint8_t)configMain.speedPercent_M4;
...
void LoadSettings_Shared()
{
	...
	iniValue = "AxisId";  // 8
	returnVal = ReturnIniValue(iniKey, iniValue);
	configSync.axisId = (int32_t)returnVal;
	configGreekKey.axisId = (int32_t)returnVal;
	configRec.axisId = (int32_t)returnVal;
	value8 = (uint8_t)configSync.axisId;
...
void LoadSettings_PagePreferences()
{
	...
	float returnVal = ReturnIniValue(iniKey, iniValue);
	returnVal >= 1 ? (configSetup.polarity_M4 = true) : (configSetup.polarity_M4 = false);
	value24 = (uint8_t)returnVal;
	...
	eePromAddress_Nextion = 460;
	returnVal = ReturnIniValue(iniKey, iniValue);
	configSetup.radialOrLinear_M4 = returnVal;
	value8 = (uint8_t)configSetup.radialOrLinear_M4;


	iniValue = "Polarity_Z";
	eePromAddress_Nextion = 460;
	returnVal = ReturnIniValue(iniKey, iniValue);
	returnVal >= 1 ? (configSetup.polarity_Z = true) : (configSetup.polarity_Z = false);
	value0 = (uint8_t)returnVal;
	...
	eePromAddress_Nextion = 176;
	returnVal = ReturnIniValue(iniKey, iniValue);
	returnVal >= 1 ? (configSetup.polarity_XAlt = true) : (configSetup.polarity_XAlt = false);
	value0 = (uint8_t)configSetup.polarity_XAlt;
	...
	eePromAddress_Nextion = 176;
	returnVal = ReturnIniValue(iniKey, iniValue);
	returnVal >= 1 ? (configSetup.polarity_X = true) : (configSetup.polarity_X = false);
	value16 = (uint8_t)configSetup.polarity_X;
	...
	returnVal = ReturnIniValue(iniKey, iniValue);
	returnVal >= 1 ? (configSetup.polarity_M3 = true) : (configSetup.polarity_M3 = false);
	value0 = (uint8_t)returnVal;
	...
	returnVal = ReturnIniValue(iniKey, iniValue);
	configSetup.radialOrLinear_M3 = (int)returnVal;
	value16 = (uint8_t)configSetup.radialOrLinear_M3;
	...
	returnVal = GetIniValue(iniKey, iniValue, eePromAddress_Nextion, false);
	configSetup.microsteps_Z = (int)returnVal;

	iniValue = "StepsPer360_Z";
	eePromAddress_Nextion = 464;
	returnVal = GetIniValue(iniKey, iniValue, eePromAddress_Nextion, false);
	configSetup.steps360_Z = (int)returnVal;

	iniValue = "DistancePer360_Z";
	eePromAddress_Nextion = 512;
	returnVal = GetIniValue(iniKey, iniValue, eePromAddress_Nextion, true);
	configSetup.distancePerRev_Z = returnVal;
	...
	returnVal = GetIniValue(iniKey, iniValue, eePromAddress_Nextion, false);
	configSetup.microsteps_M4 = (int)returnVal;

	iniValue = "StepsPer360_M4";
	eePromAddress_Nextion = 404;
	returnVal = GetIniValue(iniKey, iniValue, eePromAddress_Nextion, false);
	configSetup.steps360_M4 = (int)returnVal;

	iniValue = "GearRatio_M4";
	eePromAddress_Nextion = 380;
	returnVal = GetIniValue(iniKey, iniValue, eePromAddress_Nextion, true);
	configSetup.gearRatio_M4 = returnVal;
#ifdef DEBUG
	...
	returnVal = GetIniValue(iniKey, iniValue, eePromAddress_Nextion, true);
	configSetup.distancePerRev_M4 = returnVal;
...
void LoadSettings_PageLimits()
{
	...
	eePromAddress_Nextion = 368;
	returnVal = ReturnIniValue(iniKey, iniValue);
	configIndex_Prime.speedPercent_M3 = (int)returnVal;
	value16 = (uint8_t)configIndex_Prime.speedPercent_M3;

	iniValue = "SpeedPercentage_M4";
	eePromAddress_Nextion = 368;
	returnVal = ReturnIniValue(iniKey, iniValue);
	configIndex_Prime.speedPercent_M4 = (int)returnVal;
	value24 = (uint8_t)configIndex_Prime.speedPercent_M4;
...
void SetLimitColors(uint32_t page, int direction)
{
	String pageName = "";
	String printString = "";
