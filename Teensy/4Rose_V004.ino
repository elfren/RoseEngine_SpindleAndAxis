...
void setup()
{
	...
	accelStep_Axis_Z.setPinsInverted(false, false, configMain.enable_Axis_Z);
	...
	accelStep_Axis_B.setPinsInverted(false, false, configMain.enable_Axis_B);
	...
	//------------------------------
	teensyStep_Axis_B
		.setMaxSpeed(configSpB.maxSpd_Axis_B)       // steps/s
		.setAcceleration(configSpB.accel_Axis_B) // steps/s^2 
		.setStepPinPolarity(configMain.enable_Axis_B ? (LOW) : (HIGH)); // driver expects active low pulses
#endif // FOUR_AXES
#ifndef TWO_AXES_V2 // Three and Four axes boards
	// X Axis stepper motor initialization
	// Configure Enable X Axis stepper
	accelStep_Axis_X.setEnablePin(PIN_AXIS_X_ENABLE);
	accelStep_Axis_X.setPinsInverted(false, false, configMain.enable_Axis_X);
	...
	teensyStep_Axis_X
		...
		.setStepPinPolarity(configMain.enable_Axis_X ? (LOW) : (HIGH)); // driver expects active low pulses
#endif // TWO_AXES_V2
	...
	teensyStep_Axis_Z
		...
		.setStepPinPolarity(configMain.enable_Axis_Z ? (LOW) : (HIGH)); // driver expects active low pulses
	...
	if (sdValid)
	{
		Serial1.print("pageIndexF2.vaDisplayFile.val=1"); // Nextion may not get the first packet
		Serial1.write(0xff);
		Serial1.write(0xff);
		Serial1.write(0xff);
		delay(50);
		Serial1.print("pageIndexF2.vaDisplayFile.val=1");
		Serial1.write(0xff);
		Serial1.write(0xff);
		Serial1.write(0xff);
	}
	...
	{
		Serial1.print("pageIndexF2.vaDisplayFile.val=0"); // Nextion may not get the first packet
		Serial1.write(0xff);
		Serial1.write(0xff);
		Serial1.write(0xff);
		delay(50);
		Serial1.print("pageIndexF2.vaDisplayFile.val=0");
		Serial1.write(0xff);
		Serial1.write(0xff);
		Serial1.write(0xff);
	}
...
void loop()
{
	...
	// Enable if running from computer
	...
	//Serial1.print("pageSpZ.t6.txt="); // Nextion may not get the first packet
	//Serial1.write(0x22);
	//Serial1.print("Id:");
	...
	//#endif // FOUR_AXES
		...
		case 103: // g - 
		{
			Serial1.print("pageConfig.t19.txt="); // Nextion may not get the first packet
			Serial1.write(0x22);
...
//#endif // TWO_AXES_V2
		...
		case 217: // Ù - Enable Z (High or Low)
		{
			...
			// true (1): LOW  false (0): HIGH
			enableZ >= 1 ? (configMain.enable_Axis_Z = true) : (configMain.enable_Axis_Z = false);
			EEPROM.put(eePromAddress_Setup, configMain);
			break;
		}
		case 218: // Ú - Enable X (High or Low)
		{
			...
			// true (1): LOW  false (0): HIGH
			enableX >= 1 ? (configMain.enable_Axis_X = true) : (configMain.enable_Axis_X = false);
			EEPROM.put(eePromAddress_Setup, configMain);
			break;
		}
		case 219: // Û - Enable B (High or Low)
		{
			...
			// true (1): LOW  false (0): HIGH
			enableB >= 1 ? (configMain.enable_Axis_B = true) : (configMain.enable_Axis_B = false);
			EEPROM.put(eePromAddress_Setup, configMain);
			break;
		}
		case 220: // Ü - Limit switch Z Min
		{
			configMain.limit_Min_Z = GetSerial1Float();
			EEPROM.put(eePromAddress_Setup, configMain);
			break;
		}
		case 221: // Ý - Limit switch Z Max
		{
			configMain.limit_Max_Z = GetSerial1Float();
			EEPROM.put(eePromAddress_Setup, configMain);
			break;
		}
		case 222: // Þ - Limit switch X Min
		{
			configMain.limit_Min_X = GetSerial1Float();
			EEPROM.put(eePromAddress_Setup, configMain);
			break;
		}
		case 223: // ß - Limit switch X Max
		{
			configMain.limit_Max_X = GetSerial1Float();
			EEPROM.put(eePromAddress_Setup, configMain);
			break;
		}
		case 224: // à - Limit switch B Min
		{
			configMain.limit_Min_B = GetSerial1Float();
			EEPROM.put(eePromAddress_Setup, configMain);
			break;
		}
		case 225: // á - Limit switch B Max
		{
			configMain.limit_Max_B = GetSerial1Float();
			EEPROM.put(eePromAddress_Setup, configMain);
			break;
		}
		default:
