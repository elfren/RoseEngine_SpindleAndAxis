...
//#include "C:\Users\etfre\AppData\Local\Temp\VMBuilds\t41_rose\teensy41\Debug\pch\Arduino.h"
#include "Arduino.h"
#include "math.h"
#include <string>
#include <SPI.h>
...
// Serial1
// 
//Nextion Serial baud rate set in Nextion program.s tab
//Serial1.begin(9600); 
...
/*
		...
		////Serial1.begin(512000); //Nextion Serial baud rate set in Nextion pageMain Preinitialize Event tab
...
if (Serial1.available() > 0)
{
	...
	//Serial.print("Baud: ");
	//Serial.println("bauds=115200");
#endif // Debug
}
else
{
	Serial.println("Serial1 not found");
	...
	Serial.println("Serial2.begin(921600);");
	SerialPrint("pageSplash.t1.txt=0");
	SerialWrite(0x22);
	SerialPrint("Serial2");
	SerialWrite(0x22);
	...
	// Only Serial1 and Serial2 are available on the board with Teensy 4.1
	/*
					// Serial3
...
////	}
	...
	if (serialPortAvailable > 0)
	{
		...
		{
			Serial.print("configGreekKey.patternCount_Grk: ");
			Serial.println(configGreekKey.patternCount_Grk);
			//configGreekKey.patternCount_Grk = GetSerialInteger();
			configGreekKey.patternCount_Grk = (uint32_t)GetSerialFloat(serialId);
			EEPROM.put(eePromAddress_Grk, configGreekKey);
		...
		case 58: // : Colon - Spindle MaxSpeed
		{
			...
			switch (pageCallerId)
			{
				...
				case PAGE_ONE:
				{
					configOne.maxSpd_Spindle = maxSpeed_Spindle;
		...
		case 59: // ; Semicolon - Spindle Acceleration
		{
			...
			switch (pageCallerId)
			{
				...
				case PAGE_ONE:
				{
					...
					configOne.accel_Spindle = spindleAccel;
	#ifdef DEBUG
					Serial.print(accel_Char);
		...
		case 60: // < Less than - Write Settings to SD card
		{
			WriteSettingsToSD();
			break;
		}
		...
		{
			configRec.grow_SpindleChange = GetSerialFloat(serialId);
			EEPROM.put(eePromAddress_Rec, configRec);
#ifdef DEBUG
			Serial.print(distance_Char);
			Serial.println(configRec.grow_SpindleChange, 5);
#endif // DEBUG
			break;
		}
		...
		case 66: //B - AxisId
		{
			...
			switch (pageCallerId)
			{
				...
				case PAGE_ONE:
				{
					Serial.print("Current pageOne.axisId:  ");
					Serial.println(configOne.axisId);

					Serial.print("2.axisId:  ");
					Serial.println(axisId);
					configOne.axisId = axisId;
					EEPROM.put(eePromAddress_One, configOne);

					Serial.print("New pageOne.axisId:  ");
					Serial.println(configOne.axisId);
					break;
				}
				case PAGE_INDEX:
		...
		case 69: //E - Index by divisions or degrees
		{
			switch (configIndex_Prime.indexId)
			{
				case INDEX_1:
				{
					...
					Serial.println("End Index_1 Case 69.................");
					break;
				}
				case INDEX_2:
				{
					...
					Serial.println("End Index_2 Case 69.................");
					break;
				}
				case INDEX_3:
				{
		...
		{
			configGreekKey.patternCount_Program = GetSerialFloat(serialId);//  GetSerialInteger();
#ifdef DEBUG
		...
		{
			configGreekKey.segmentLength_Program = GetSerialFloat(serialId);
			EEPROM.put(eePromAddress_Grk, configGreekKey);
#ifdef DEBUG
		...
		{
			configSync.spindle_RevolutionsOrDegrees = GetSerialInteger();
			EEPROM.put(eePromAddress_Sync, configSync);
			Serial.print("RevolutionsOrDegrees:  ");
		...
		case 98: // b - Deprecated Test EEPROM settings Setup screen
		{
			Serial.println("Deprecated---98");
			//Serial.print("pageCallerId: ");
			//Serial.println(pageCallerId);
		...
		case 100: // d - pageMulti/One Clockwise
		{
		...
		case 101: // e - pageMulti/One CounterClockwise
		{
		...
		{
			configSetup.radialOrLinear_Axis_M4 = (uint32_t)GetSerialInteger();
			// Radial = 0, Linear = 1
		...
		{
			//uint32_t nueValue = GetSerialInteger();
			uint32_t nueValue = (uint32_t)GetSerialFloat(serialId);
			configSetup.auxillaryPin = nueValue;
			//configSetup.auxillaryPin = (int)GetSerialFloat(serialId);
		...
		case 182: // ¶ - Greek Key: Pattern Type  (2 through 7)
		{
			...
			EEPROM.put(eePromAddress_Grk, configGreekKey);
			Serial.print("configGreekKey.patternId: ");
		...
		case 185: // ¹ - Axis Max speed
		{
			...
			switch (pageCallerId)
			{
				...
				case PAGE_ONE:
				{
					Serial.print("pageOne-configOne.axisId: ");
					Serial.println(pageCallerId);
					switch (configOne.axisId)
					{
						case ID_AXIS_Z:
						{
							configOne.maxSpd_Z = maxSpd_axis;
		#ifdef DEBUG
							Serial.print("pageOne-Z ");
							Serial.print(maxSpd_Char);
							Serial.println(configOne.maxSpd_Z);
		#endif // DEBUG
							break;
						}
						case ID_AXIS_X:
						{
							configOne.maxSpd_X = maxSpd_axis;
		#ifdef DEBUG
							Serial.print("pageOne-X ");
							Serial.print(maxSpd_Char);
							Serial.println(configOne.maxSpd_X);
		#endif // DEBUG
							break;
						}
						case ID_AXIS_3:
						{
							configOne.maxSpd_M3 = maxSpd_axis;
		#ifdef DEBUG
							Serial.print("pageOne-M3 ");
							Serial.print(maxSpd_Char);
							Serial.println(configOne.maxSpd_M3);
		#endif // DEBUG
							break;
						}
						case ID_AXIS_4:
						{
							configOne.maxSpd_M4 = maxSpd_axis;
		#ifdef DEBUG
							Serial.print("pageOne-M4 ");
							Serial.print(maxSpd_Char);
							Serial.println(configOne.maxSpd_M4);
		#endif // DEBUG
							break;
						}
		...
		case 186: // º - Axis Accel
		{
			...
			switch (pageCallerId)
			{
				...
				case PAGE_ONE:
				{
					...
					case ID_AXIS_Z:
					{
						configOne.accel_Z = accel_axis;
						break;
					}
					case ID_AXIS_X:
					{
						configOne.accel_X = accel_axis;
						break;
					}
					case ID_AXIS_3:
					{
						configOne.accel_M3 = accel_axis;
	#ifdef DEBUG
						Serial.print(accel_Char);
						Serial.println(configOne.accel_M3);
	#endif // DEBUG
						break;
					}
					case ID_AXIS_4:
					{
						configOne.accel_M4 = accel_axis;
	#ifdef DEBUG
						Serial.print(accel_Char);
						Serial.println(configOne.accel_M4);
	#endif // DEBUG
						break;
					}
					} // end switch (pageCallerId)
		...
		{
			Serial.print("55. fileNameIndex: ");
			Serial.println(fileNameIndex);
			GetFilenameFromSerial();
			Serial.print("65. fileNameIndex: ");
			Serial.println(fileNameIndex);
			lastFileIndex = -1;
			break;
		}
		...
		{
			//int32_t direction = GetSerialInteger();
			int32_t direction = GetSerialIntegerOnly();
			...
			switch (configMulti.axisId)
			{
				case ID_SPINDLE:
				{
					configMulti.direction_Spindle = newDir;
					Serial.print("Spindle ");
					break;
				}
				case ID_AXIS_Z:
				{
					configMulti.direction_Z = newDir;
					Serial.print("..................................Z ");
				...
				case ID_AXIS_X:
				{
					configMulti.direction_X = newDir;
					Serial.print("X ");
					break;
				}
				case ID_AXIS_3:
				{
					configMulti.direction_M3 = newDir;
					Serial.print("M3 ");
					break;
				}
				case ID_AXIS_4:
				{
					configMulti.direction_M4 = newDir;
					Serial.print("M4 ");
					break;
				}
			}
		...
		{
			configSetup.radialOrLinear_Axis_M3 = (uint32_t)GetSerialInteger();
			// Radial = 0, Linear = 1
		...
		case 200: // È - pageMulti: Multi Axes Checked
		{
			...
			uint32_t newCheck = GetSerialIntegerOnly();

			switch (configMulti.axisId)
			{
			case ID_SPINDLE:
			{
				configMulti.checked_Spindle = newCheck;
				Serial.print("Spindle ");
				break;
			}
			case ID_AXIS_Z:
			{
				configMulti.checked_Z = newCheck;
				Serial.print("..................................Z ");
				Serial.println(configMulti.direction_Z);
				break;
			}
			case ID_AXIS_X:
			{
				configMulti.checked_X = newCheck;
				Serial.print("X ");
				break;
			}
			case ID_AXIS_3:
			{
				configMulti.checked_M3 = newCheck;
				Serial.print("M3 ");
				break;
			}
			case ID_AXIS_4:
			{
				configMulti.checked_M4 = newCheck;
				Serial.print("M4 ");
				break;
			}
		...
		{
			configRec.grow_AxisChange = GetSerialFloat(serialId);
			EEPROM.put(eePromAddress_Rec, configRec);
#ifdef DEBUG
			Serial.print(distance_Char);
			Serial.println(configRec.grow_AxisChange, 5);
#endif // DEBUG
			break;
		}
		...
		case 211: // Ó - Leadscrew Hand
		{
			int32_t id = GetSerialIntegerOnly();
			switch (id)
			{
				case ID_AXIS_Z: // Z
				{
					configSetup.leadscrewHand_Z = id;
					break;
				}
				case ID_AXIS_X: // X
				{
					configSetup.leadscrewHand_X = id;
					break;
				}
				case ID_AXIS_ALTX: // Z
				{
					configSetup.leadscrewHand_AltX = id;
					break;
				}
				case ID_AXIS_3: // M3
				{
					configSetup.leadscrewHand_M3 = id;
					break;
				}
				case ID_AXIS_4: // M4
				{
					configSetup.leadscrewHand_M4 = id;
					break;
				}
			}
		...
		{
			int polaritySpindle = GetSerialIntegerOnly();
		...
		{
			configGreekKey.countPatternPer360_Program = GetSerialFloat(serialId);
			EEPROM.put(eePromAddress_Grk, configGreekKey);
#ifdef DEBUG
			Serial.print(patternCount_Char);
			Serial.println(configGreekKey.countPatternPer360_Program);
#endif // DEBUG
			break;
		}
...
//#endif // DEBUG
		...
		case 247: // ÷ - Do Greek Key (Left and Right) 
		{
			...
			if (configSetup.keepSteppersEnabled == 1)
			{
				SerialPrint("pageGrk.bt10.val=1"); // 1 = enabled
				SerialPrint("\xFF\xFF\xFF");
				SerialPrint("pageGrk.bt11.val=1"); // 1 = enabled
				SerialPrint("\xFF\xFF\xFF");
			}
			else
			{
		...
		{
			configGreekKey.segmentLength_Grk = GetSerialFloat(serialId);
			EEPROM.put(eePromAddress_Grk, configGreekKey);
#ifdef DEBUG
			Serial.print(segmentLength_Char);
		...
		{
			configGreekKey.countPatternPer360_Grk = GetSerialFloat(serialId);
			EEPROM.put(eePromAddress_Grk, configGreekKey);
#ifdef DEBUG
			Serial.print(patternCount_Char);
